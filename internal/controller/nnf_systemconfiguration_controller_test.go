...
var _ = Describe("Adding taints and labels to nodes", func() {
	...
	verifyTaintsAndLabels := func(node *corev1.Node) string {
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(labels).To(HaveKeyWithValue(nnfv1alpha4.RabbitNodeSelectorLabel, "true"))
			g.Expect(labels).To(HaveKeyWithValue(nnfv1alpha4.TaintsAndLabelsCompletedLabel, "true"))
	...
	When("a node is added", func() {
		...
		It("should not taint and label a non-Rabbit node", func() {
			...
			Consistently(func(g Gomega) {
				...
				labels := tnode.GetLabels()
				g.Expect(labels).ToNot(HaveKey(nnfv1alpha4.RabbitNodeSelectorLabel))
				g.Expect(labels).ToNot(HaveKey(nnfv1alpha4.TaintsAndLabelsCompletedLabel))
