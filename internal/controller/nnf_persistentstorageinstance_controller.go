...
func (r *PersistentStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	statusUpdater := updater.NewStatusUpdater[*dwsv1alpha4.PersistentStorageInstanceStatus](persistentStorage)
	...
	if !persistentStorage.GetDeletionTimestamp().IsZero() {
		...
		deleteStatus, err := dwsv1alpha4.DeleteChildren(ctx, r.Client, r.getChildObjects(), persistentStorage)
		if err != nil {
	...
	if persistentStorage.Status.State == "" {
		persistentStorage.Status.State = dwsv1alpha4.PSIStateCreating
	}
	...
	if argsMap["type"] == "lustre" && len(pinnedProfile.Data.LustreStorage.StandaloneMGTPoolName) > 0 {
		if _, exists := argsMap["capacity"]; exists {
			return ctrl.Result{}, dwsv1alpha4.NewResourceError("").WithUserMessage("creating persistent MGT does not accept 'capacity' argument").WithFatal().WithUser()
		}
		...
		if _, exists := argsMap["capacity"]; !exists {
			return ctrl.Result{}, dwsv1alpha4.NewResourceError("").WithUserMessage("creating persistent storage requires 'capacity' argument").WithFatal().WithUser()
		}
	}
	...
	if persistentStorage.Spec.State == dwsv1alpha4.PSIStateDestroying {
		if len(persistentStorage.Spec.ConsumerReferences) == 0 {
			persistentStorage.Status.State = dwsv1alpha4.PSIStateDestroying
		}
	} else if persistentStorage.Spec.State == dwsv1alpha4.PSIStateActive {
		...
		if complete == true {
			persistentStorage.Status.State = dwsv1alpha4.PSIStateActive
		}
	}
...
func (r *PersistentStorageReconciler) createServers(ctx context.Context, persistentStorage *dwsv1alpha4.PersistentStorageInstance) (*dwsv1alpha4.Servers, error) {
	...
	result, err := ctrl.CreateOrUpdate(ctx, r.Client, server,
		func() error {
			dwsv1alpha4.AddOwnerLabels(server, persistentStorage)
			dwsv1alpha4.AddPersistentStorageLabels(server, persistentStorage)
...
func (r *PersistentStorageReconciler) getChildObjects() []dwsv1alpha4.ObjectList {
...
func (r *PersistentStorageReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&dwsv1alpha4.PersistentStorageInstance{}).
