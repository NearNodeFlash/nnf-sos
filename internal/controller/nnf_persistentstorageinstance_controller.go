...
func (r *PersistentStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	if argsMap["type"] == "lustre" && len(pinnedProfile.Data.LustreStorage.MgtOptions.StandaloneMGTPoolName) > 0 {
		...
		if _, ok := labels[nnfv1alpha11.StandaloneMGTLabel]; !ok {
			labels[nnfv1alpha11.StandaloneMGTLabel] = pinnedProfile.Data.LustreStorage.MgtOptions.StandaloneMGTPoolName
	...
	if persistentStorage.Spec.State == dwsv1alpha7.PSIStateDestroying {
		...
		if complete == true {
			...
			if nnfStorage.Status.Health != nnfv1alpha11.NnfStorageHealthHealthy {
