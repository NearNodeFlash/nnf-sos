...
func (r *PersistentStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	if !persistentStorage.GetDeletionTimestamp().IsZero() {
		...
		deleteStatus, err := dwsv1alpha3.DeleteChildren(ctx, r.Client, r.ChildObjects, persistentStorage)
		if err != nil {
	...
	if argsMap["type"] == "lustre" && len(pinnedProfile.Data.LustreStorage.StandaloneMGTPoolName) > 0 {
		...
		if _, ok := labels[nnfv1alpha3.StandaloneMGTLabel]; !ok {
			labels[nnfv1alpha3.StandaloneMGTLabel] = pinnedProfile.Data.LustreStorage.StandaloneMGTPoolName
