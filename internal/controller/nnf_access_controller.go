...
func (r *NnfAccessReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	if access.Status.State == "mounted" {
		...
		if err != nil {
			return ctrl.Result{}, dwsv1alpha7.NewResourceError("").WithError(err).WithUserMessage("unable to unmount file system from client nodes")
		}
	}
...
func (r *NnfAccessReconciler) mount(ctx context.Context, access *nnfv1alpha10.NnfAccess, clientList []string, storageMapping map[string][]dwsv1alpha7.ClientMountInfo) (*ctrl.Result, error) {
	...
	if err != nil {
		return nil, dwsv1alpha7.NewResourceError("unable to lock storage").WithError(err)
	}
	...
	if err != nil {
		...
		return nil, dwsv1alpha7.NewResourceError("unable to add endpoints to NnfNodeStorage").WithError(err)
	}
	...
	if err != nil {
		return nil, dwsv1alpha7.NewResourceError("unable to check endpoints for NnfNodeStorage").WithError(err)
	}
	...
	if err != nil {
		...
		return nil, dwsv1alpha7.NewResourceError("unable to create ClientMount resources").WithError(err)
	}

	// Aggregate the status from all the ClientMount resources
	ready, err = r.getClientMountStatus(ctx, access, clientList)
	if err != nil {
		return nil, dwsv1alpha7.NewResourceError("unable to check ClientMount status").WithError(err)
	}
...
func (r *NnfAccessReconciler) unmount(ctx context.Context, access *nnfv1alpha10.NnfAccess, clientList []string, storageMapping map[string][]dwsv1alpha7.ClientMountInfo) (*ctrl.Result, error) {
	...
	// Aggregate the status from all the ClientMount resources.
	// getClientMountStatus will ignore offline/fenced/disabled nodes during unmount,
	// allowing teardown to complete even when some nodes are unavailable.
	ready, err := r.getClientMountStatus(ctx, access, clientList)
	if err != nil {
	...
	err = r.removeBlockStorageAccess(ctx, access, storageMapping)
	if err != nil {
		return nil, dwsv1alpha7.NewResourceError("unable to remove NnfNodeStorage endpoints").WithError(err)
	}
	...
	// Unlock the NnfStorage so it can be used by another NnfAccess
	if err = r.unlockStorage(ctx, access); err != nil {
		return nil, dwsv1alpha7.NewResourceError("unable to unlock storage").WithError(err)
	}
...
func (r *NnfAccessReconciler) lockStorage(ctx context.Context, access *nnfv1alpha10.NnfAccess) (bool, error) {
...
func (r *NnfAccessReconciler) unlockStorage(ctx context.Context, access *nnfv1alpha10.NnfAccess) error {
	nnfStorage := &nnfv1alpha10.NnfStorage{}
...
func (r *NnfAccessReconciler) getClientList(ctx context.Context, access *nnfv1alpha10.NnfAccess) ([]string, error) {
...
func (r *NnfAccessReconciler) getClientListFromClientReference(ctx context.Context, access *nnfv1alpha10.NnfAccess) ([]string, error) {
	computes := &dwsv1alpha7.Computes{}

	if access.Spec.ClientReference.Kind != reflect.TypeOf(dwsv1alpha7.Computes{}).Name() {
...
func (r *NnfAccessReconciler) getClientListFromStorageReference(ctx context.Context, access *nnfv1alpha10.NnfAccess) ([]string, error) {
...
func (r *NnfAccessReconciler) mapClientStorage(ctx context.Context, access *nnfv1alpha10.NnfAccess, clients []string) (map[string][]dwsv1alpha7.ClientMountInfo, error) {
	nnfStorage := &nnfv1alpha10.NnfStorage{}
	...
	for i := range nnfStorage.Spec.AllocationSets {
		var storageMapping map[string][]dwsv1alpha7.ClientMountInfo
		var err error
...
// mount information
func (r *NnfAccessReconciler) mapClientNetworkStorage(ctx context.Context, access *nnfv1alpha10.NnfAccess, clients []string, nnfStorage *nnfv1alpha10.NnfStorage, setIndex int) (map[string][]dwsv1alpha7.ClientMountInfo, error) {
	storageMapping := make(map[string][]dwsv1alpha7.ClientMountInfo)

	for _, client := range clients {
		mountInfo := dwsv1alpha7.ClientMountInfo{}
		mountInfo.Type = nnfStorage.Spec.FileSystemType
		mountInfo.TargetType = "directory"
		mountInfo.MountPath = access.Spec.MountPath
		mountInfo.Device.Type = dwsv1alpha7.ClientMountDeviceTypeLustre
		mountInfo.Device.Lustre = &dwsv1alpha7.ClientMountDeviceLustre{}
		...
		mountInfo.Device.DeviceReference = &dwsv1alpha7.ClientMountDeviceReference{
...
func (r *NnfAccessReconciler) mapClientLocalStorage(ctx context.Context, access *nnfv1alpha10.NnfAccess, clients []string, nnfStorage *nnfv1alpha10.NnfStorage, setIndex int) (map[string][]dwsv1alpha7.ClientMountInfo, error) {
	...
	existingStorage := make(map[string][]dwsv1alpha7.ClientMountInfo)
	...
	for nodeName, storageCount := range storageCountMap {
		...
		matchLabels[nnfv1alpha10.AllocationSetLabel] = allocationSetSpec.Name
		...
		if len(nnfNodeStorageList.Items) != storageCount.instanceCount {
			return nil, dwsv1alpha7.NewResourceError("incorrect number of NnfNodeStorages. found %d. Needed %d.", len(nnfNodeStorageList.Items), storageCount.instanceCount).WithMajor()
		}
		...
		for _, nnfNodeStorage := range nnfNodeStorageList.Items {
			...
			if err := r.Get(ctx, client.ObjectKeyFromObject(nnfNodeBlockStorage), nnfNodeBlockStorage); err != nil {
				return nil, dwsv1alpha7.NewResourceError("unable to get nnfNodeBlockStorage: %v", client.ObjectKeyFromObject(nnfNodeBlockStorage)).WithError(err).WithMajor()
			}
			...
			for i := 0; i < nnfNodeStorage.Spec.Count; i++ {
				...
				mountInfo.Device.DeviceReference = &dwsv1alpha7.ClientMountDeviceReference{}
				mountInfo.Device.DeviceReference.ObjectReference.Kind = reflect.TypeOf(nnfv1alpha10.NnfNodeStorage{}).Name()
				...
				if access.Spec.ClientReference == (corev1.ObjectReference{}) {
					...
					mountInfo.Device.Type = dwsv1alpha7.ClientMountDeviceTypeReference
				} else {
					mountInfo.MountPath = access.Spec.MountPath
					mountInfo.Device.Type = dwsv1alpha7.ClientMountDeviceTypeLVM
					mountInfo.Device.LVM = &dwsv1alpha7.ClientMountDeviceLVM{}
					...
					mountInfo.Device.LVM.DeviceType = dwsv1alpha7.ClientMountLVMDeviceTypeNVMe
					...
					for _, nvmeDesc := range nnfNodeBlockStorage.Status.Allocations[blockAllocationSetIndex].Devices {
						mountInfo.Device.LVM.NVMeInfo = append(mountInfo.Device.LVM.NVMeInfo, dwsv1alpha7.ClientMountNVMeDesc{DeviceSerial: nvmeDesc.NQN, NamespaceID: nvmeDesc.NamespaceId})
					}
				}
	...
	storageMapping := make(map[string][]dwsv1alpha7.ClientMountInfo)
	for _, client := range clients {
		storageMapping[client] = []dwsv1alpha7.ClientMountInfo{}
	}
	...
	for storageName := range existingStorage {
		...
		for _, client := range clients {
			...
			if len(existingStorage[storageName]) == 0 {
				return nil, dwsv1alpha7.NewResourceError("").WithUserMessage("invalid matching between clients and storage. Too many clients for storage").WithWLM().WithFatal()
			}
			...
			case "all":
				...
				existingStorage[storageName] = []dwsv1alpha7.ClientMountInfo{}
			case "single":
			...
			case "shared":
				...
				storageMapping[client] = append(storageMapping[client], existingStorage[storageName][0])
			default:
				return nil, dwsv1alpha7.NewResourceError("invalid target type '%s'", access.Spec.Target).WithFatal()
			}
		}
	}
...
func (r *NnfAccessReconciler) addBlockStorageAccess(ctx context.Context, access *nnfv1alpha10.NnfAccess, storageMapping map[string][]dwsv1alpha7.ClientMountInfo) error {
	...
	for client, storageList := range storageMapping {
		for _, mount := range storageList {
			...
			if mount.Device.DeviceReference.ObjectReference.Kind != reflect.TypeOf(nnfv1alpha10.NnfNodeStorage{}).Name() {
				continue
			}
...
func (r *NnfAccessReconciler) getBlockStorageAccessStatus(ctx context.Context, access *nnfv1alpha10.NnfAccess, storageMapping map[string][]dwsv1alpha7.ClientMountInfo) (bool, error) {
	...
	nnfNodeBlockStorages := []nnfv1alpha10.NnfNodeBlockStorage{}
	...
	for _, nnfNodeBlockStorage := range nnfNodeBlockStorages {
		if nnfNodeBlockStorage.Status.Error != nil {
			return false, dwsv1alpha7.NewResourceError("Node: %s", nnfNodeBlockStorage.GetNamespace()).WithError(nnfNodeBlockStorage.Status.Error)
		}
	}
...
func (r *NnfAccessReconciler) removeBlockStorageAccess(ctx context.Context, access *nnfv1alpha10.NnfAccess, storageMapping map[string][]dwsv1alpha7.ClientMountInfo) error {
...
func (r *NnfAccessReconciler) manageClientMounts(ctx context.Context, access *nnfv1alpha10.NnfAccess, storageMapping map[string][]dwsv1alpha7.ClientMountInfo) error {
	...
	if access.Spec.StorageReference.Kind != reflect.TypeOf(nnfv1alpha10.NnfStorage{}).Name() {
		return dwsv1alpha7.NewResourceError("invalid StorageReference kind %s", access.Spec.StorageReference.Kind).WithFatal()
	}
	...
	if err := r.Get(ctx, client.ObjectKeyFromObject(nnfStorage), nnfStorage); err != nil {
		return dwsv1alpha7.NewResourceError("could not get NnfStorage %v", client.ObjectKeyFromObject(nnfStorage)).WithError(err).WithMajor()
	}
	...
	for clientName, storageList := range storageMapping {
		...
		g.Go(func() error {
			...
			result, err := ctrl.CreateOrUpdate(ctx, r.Client, clientMount,
				func() error {
					dwsv1alpha7.InheritParentLabels(clientMount, access)
					dwsv1alpha7.AddOwnerLabels(clientMount, access)
					...
					clientMount.Spec.DesiredState = dwsv1alpha7.ClientMountState(access.Spec.DesiredState)
...
func (r *NnfAccessReconciler) getClientMountStatus(ctx context.Context, access *nnfv1alpha10.NnfAccess, clientList []string) (bool, error) {
	...
	if err := r.List(ctx, clientMountList, listOptions...); err != nil {
		return false, dwsv1alpha7.NewResourceError("could not list ClientMounts").WithError(err)
	}
	...
	clientMounts := []dwsv1alpha7.ClientMount{}
	...
	if len(childTimeoutString) > 0 {
		...
		for _, clientMount := range clientMounts {
			...
			if clientMount.GetCreationTimestamp().Add(time.Duration(time.Duration(childTimeout) * time.Second)).Before(time.Now()) {
				return false, dwsv1alpha7.NewResourceError("Node: %s: ClientMount has not been reconciled after %d seconds", clientMount.GetNamespace(), childTimeout).WithMajor()
			}
	...
	for _, clientMount := range clientMounts {
		// Check if status hasn't been populated yet
		statusNotReady := len(clientMount.Status.Mounts) != len(clientMount.Spec.Mounts)

		// Check if all mounts are ready and in the correct state
		allMountsReady := !statusNotReady
		if !statusNotReady {
		...
		// If mounts aren't ready, check if this compute node is offline/fenced/disabled
		...
		if !allMountsReady || access.Spec.DesiredState == "unmounted" {
			offline, reason, err := r.checkOfflineCompute(ctx, access, &clientMount)
			if err != nil {
				log.Error(err, "checkOfflineCompute failed", "node", clientMount.GetNamespace())
				return false, err
			}

			log.Info("checkOfflineCompute result", "node", clientMount.GetNamespace(), "offline", offline, "reason", reason, "allMountsReady", allMountsReady, "desiredState", access.Spec.DesiredState)

			// During unmount: ignore any offline/fenced/disabled node since it can't respond.
			// The filesystem won't be remounted if the compute comes back since spec.desiredState is "unmounted".
			// During mount: only ignore nodes that are "disabled" in SystemStatus. Nodes that are
			// offline or fenced but not disabled must complete their mount or stall the workflow.
			if offline {
				if access.Spec.DesiredState == "unmounted" {
					log.Info("ignoring ClientMount from unavailable compute node during unmount", "node", clientMount.GetNamespace(), "reason", reason)
					continue
				} else if reason == "disabled" {
					// During mount, only ignore nodes disabled in SystemStatus
					log.Info("ignoring ClientMount from disabled compute node during mount", "node", clientMount.GetNamespace())
					continue
				}
				// For mount operations with offline/fenced (but not disabled) nodes, fall through to return false
			}
		}

		// If mounts aren't ready and node isn't being ignored, return false
		if !allMountsReady {
			return false, nil
		}
	}

	// Check if the number of ClientMounts matches the expected number, accounting for nodes
	// that are unavailable. During unmount, we ignore all unavailable nodes (offline, fenced, disabled).
	// During mount, we only ignore nodes that are disabled in SystemStatus.
	expectedClientMounts := len(clientList)

	// Categorize nodes by their unavailability reason
	offlineNodes := []string{}
	fencedNodes := []string{}
	disabledNodes := []string{}

	for _, clientName := range clientList {
		// Create a dummy ClientMount to use for the offline check
		dummyClientMount := dwsv1alpha7.ClientMount{
			ObjectMeta: metav1.ObjectMeta{
				Name:      clientMountName(access),
				Namespace: clientName,
			},
		...
		offline, reason, err := r.checkOfflineCompute(ctx, access, &dummyClientMount)
		if err != nil {
			log.Error(err, "failed to check if compute is offline", "compute", clientName)
		...
		if offline {
			switch reason {
			case "offline":
				offlineNodes = append(offlineNodes, clientName)
			case "fenced":
				fencedNodes = append(fencedNodes, clientName)
			case "disabled":
				disabledNodes = append(disabledNodes, clientName)
			}
		}
	}

	// Calculate nodes to ignore based on operation type:
	// - Unmount: ignore all unavailable nodes (offline, fenced, disabled) for status checking,
	//   but DON'T reduce expectedClientMounts because their ClientMounts still exist from mount time
	// - Mount: only ignore disabled nodes (offline/fenced must complete or stall), and
	//   reduce expectedClientMounts because disabled nodes don't get ClientMounts created
	var ignoredCount int
	if access.Spec.DesiredState == "unmounted" {
		// During unmount, log ignored nodes but don't reduce expected count
		// because their ClientMounts were created during mount and still exist
		totalIgnored := len(offlineNodes) + len(fencedNodes) + len(disabledNodes)
		if totalIgnored > 0 {
			log.Info("ignoring unavailable compute nodes during unmount", "offline", offlineNodes, "fenced", fencedNodes, "disabled", disabledNodes)
		}
		// ignoredCount stays 0 - don't subtract from expectedClientMounts
	} else {
		// During mount, disabled nodes don't get ClientMounts created
		ignoredCount = len(disabledNodes)
		if ignoredCount > 0 {
			log.Info("ignoring disabled compute nodes during mount", "disabled", disabledNodes)
		}
	}
	expectedClientMounts -= ignoredCount

	if len(clientMounts) != expectedClientMounts {
		if access.GetDeletionTimestamp().IsZero() {
			log.Info("unexpected number of ClientMounts", "found", len(clientMounts), "expected", expectedClientMounts, "original", len(clientList), "ignored", ignoredCount)
		}
		return false, nil
	}
	return true, nil
...
func clientMountName(access *nnfv1alpha10.NnfAccess) string {
...
func (r *NnfAccessReconciler) removeOfflineClientMounts(ctx context.Context, nnfAccess *nnfv1alpha10.NnfAccess) error {
	...
	clientMountList := &dwsv1alpha7.ClientMountList{}
	matchLabels := dwsv1alpha7.MatchingOwner(nnfAccess)
	...
	for _, clientMount := range clientMountList.Items {
		offline, reason, err := r.checkOfflineCompute(ctx, nnfAccess, &clientMount)
		if err != nil {
			return err
		}
		...
		log.Info("removing finalizer from unavailable compute node", "node", clientMount.GetNamespace(), "reason", reason)
		...
		if err := r.Update(ctx, &clientMount); err != nil {
			if !apierrors.IsConflict(err) {
				return dwsv1alpha7.NewResourceError("could not update ClientMount to remove finalizer for offline compute: %v", client.ObjectKeyFromObject(&clientMount)).WithError(err)
			}
		}
	}
...
// compute node, and checks whether the compute node is marked as "Offline" or "Fenced" in the Storage status.
// It also checks the SystemStatus resource to see if the compute node is marked as Disabled.
// Returns true if offline/unavailable, and a string indicating the reason ("offline", "fenced", "disabled", or "")
func (r *NnfAccessReconciler) checkOfflineCompute(ctx context.Context, nnfAccess *nnfv1alpha10.NnfAccess, clientMount *dwsv1alpha7.ClientMount) (bool, string, error) {
	log := r.Log.WithValues("NnfAccess", client.ObjectKeyFromObject(nnfAccess))

	if nnfAccess.Spec.ClientReference == (corev1.ObjectReference{}) {
		log.V(1).Info("checkOfflineCompute: no ClientReference, skipping", "compute", clientMount.GetNamespace())
		return false, "", nil
	}
	...
	if err != nil {
		log.Error(err, "checkOfflineCompute: failed to get rabbit from clientMount", "compute", computeName)
		return false, "", err
	}

	log.V(1).Info("checkOfflineCompute: found rabbit for compute", "compute", computeName, "rabbit", rabbitName)
	...
	if err := r.Get(ctx, client.ObjectKeyFromObject(storage), storage); err != nil {
		log.Error(err, "checkOfflineCompute: failed to get Storage", "compute", computeName, "rabbit", rabbitName)
	...
	for _, compute := range storage.Status.Access.Computes {
		if compute.Name == computeName {
			log.Info("checkOfflineCompute: found compute in Storage", "compute", computeName, "status", compute.Status)
			switch compute.Status {
			case dwsv1alpha7.OfflineStatus:
				return true, "offline", nil
			case dwsv1alpha7.FencedStatus:
				return true, "fenced", nil
			}
			break
		}
	}
...
func (r *NnfAccessReconciler) getRabbitFromClientMount(ctx context.Context, clientMount *dwsv1alpha7.ClientMount) (string, error) {
	...
	if len(clientMount.Spec.Mounts) > 0 && clientMount.Spec.Mounts[0].Device.DeviceReference.ObjectReference.Kind == reflect.TypeOf(nnfv1alpha10.NnfNodeStorage{}).Name() {
		rabbitName := clientMount.Spec.Mounts[0].Device.DeviceReference.ObjectReference.Namespace
		r.Log.V(1).Info("getRabbitFromClientMount: found rabbit from NnfNodeStorage reference", "compute", computeName, "rabbit", rabbitName)
		return rabbitName, nil
	}

	rabbitName, err := helpers.GetRabbitFromCompute(ctx, r.Client, computeName)
	if err != nil {
		r.Log.Error(err, "getRabbitFromClientMount: failed to get rabbit from SystemConfiguration", "compute", computeName)
		return "", err
	}
	r.Log.V(1).Info("getRabbitFromClientMount: found rabbit from SystemConfiguration", "compute", computeName, "rabbit", rabbitName)
	return rabbitName, nil
}
...
func (r *NnfAccessReconciler) ComputesEnqueueRequests(ctx context.Context, o client.Object) []reconcile.Request {
	...
	ownerName, exists := labels[dwsv1alpha7.OwnerNameLabel]
	if !exists {
		return []reconcile.Request{}
	}

	ownerNamespace, exists := labels[dwsv1alpha7.OwnerNamespaceLabel]
	if !exists {
		return []reconcile.Request{}
	}

	ownerKind, exists := labels[dwsv1alpha7.OwnerKindLabel]
	if !exists {
		return []reconcile.Request{}
	}
	...
	listOptions := []client.ListOption{
		client.MatchingLabels(map[string]string{
			...
			dwsv1alpha7.OwnerNamespaceLabel: ownerNamespace,
		}),
	}
...
func (r *NnfAccessReconciler) getChildObjects() []dwsv1alpha7.ObjectList {
