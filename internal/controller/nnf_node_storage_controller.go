...
func (r *NnfNodeStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	// the r.Update()
	statusUpdater := updater.NewStatusUpdater[*nnfv1alpha11.NnfNodeStorageStatus](nnfNodeStorage)
	...
	if len(nnfNodeStorage.Status.Allocations) == 0 {
		nnfNodeStorage.Status.Allocations = make([]nnfv1alpha11.NnfNodeStorageAllocationStatus, nnfNodeStorage.Spec.Count)
		for i := range nnfNodeStorage.Status.Allocations {
...
func (r *NnfNodeStorageReconciler) deleteAllocation(ctx context.Context, nnfNodeStorage *nnfv1alpha11.NnfNodeStorage, index int) (*ctrl.Result, error) {
...
func (r *NnfNodeStorageReconciler) checkAllocations(ctx context.Context, nnfNodeStorage *nnfv1alpha11.NnfNodeStorage, blockDevices []blockdevice.BlockDevice) error {
	...
	for index, blockDevice := range blockDevices {
		...
		if healthy {
			allocationStatus.Health = nnfv1alpha11.NnfStorageHealthHealthy
			continue
		}
		allocationStatus.Health = nnfv1alpha11.NnfStorageHealthDegraded
		nnfNodeStorage.Status.Health = nnfv1alpha11.NnfStorageHealthDegraded
		overallHealth = nnfv1alpha11.NnfStorageHealthDegraded
...
func (r *NnfNodeStorageReconciler) createAllocations(ctx context.Context, nnfNodeStorage *nnfv1alpha11.NnfNodeStorage, blockDevices []blockdevice.BlockDevice, fileSystems []filesystem.FileSystem) (*ctrl.Result, error) {
...
func (r *NnfNodeStorageReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	builder := ctrl.NewControllerManagedBy(mgr).
		...
		For(&nnfv1alpha11.NnfNodeStorage{}).
		...
		Watches(&nnfv1alpha11.NnfNodeBlockStorage{}, handler.EnqueueRequestsFromMapFunc(nnfNameMapFunc))
