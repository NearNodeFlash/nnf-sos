...
func getPinnedStorageProfileFromLabel(ctx context.Context, clnt client.Client, object metav1.Object) (*nnfv1alpha8.NnfStorageProfile, error) {
	...
	if !okName {
		return nil, dwsv1alpha5.NewResourceError("unable to find %s label", nnfv1alpha8.PinnedStorageProfileLabelName).WithFatal()
	}
	pinnedNamespace, okNamespace := labels[nnfv1alpha8.PinnedStorageProfileLabelNameSpace]
	if !okNamespace {
		return nil, dwsv1alpha5.NewResourceError("unable to find %s label", nnfv1alpha8.PinnedStorageProfileLabelNameSpace).WithFatal()
	}
