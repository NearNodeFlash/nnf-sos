...
func findProfileToUse(ctx context.Context, clnt client.Client, args map[string]string) (*nnfv1alpha2.NnfStorageProfile, error) {
	var profileName string

	nnfStorageProfile := &nnfv1alpha2.NnfStorageProfile{}
	...
	if present == false {
		nnfStorageProfiles := &nnfv1alpha2.NnfStorageProfileList{}
...
func findPinnedProfile(ctx context.Context, clnt client.Client, namespace string, pinnedName string) (*nnfv1alpha2.NnfStorageProfile, error) {
...
func createPinnedProfile(ctx context.Context, clnt client.Client, clntScheme *runtime.Scheme, args map[string]string, owner metav1.Object, pinnedName string) (*nnfv1alpha2.NnfStorageProfile, error) {
...
func addPinnedStorageProfileLabel(object metav1.Object, nnfStorageProfile *nnfv1alpha2.NnfStorageProfile) {
	labels := object.GetLabels()
	if labels == nil {
		labels = make(map[string]string)
	}

	labels[nnfv1alpha2.PinnedStorageProfileLabelName] = nnfStorageProfile.GetName()
	labels[nnfv1alpha2.PinnedStorageProfileLabelNameSpace] = nnfStorageProfile.GetNamespace()
	object.SetLabels(labels)
}

// getPinnedStorageProfileFromLabel finds the pinned storage profile via the labels on the
// specified resource.
func getPinnedStorageProfileFromLabel(ctx context.Context, clnt client.Client, object metav1.Object) (*nnfv1alpha2.NnfStorageProfile, error) {
	labels := object.GetLabels()
	if labels == nil {
		return nil, dwsv1alpha2.NewResourceError("unable to find labels").WithFatal()
	}

	pinnedName, okName := labels[nnfv1alpha2.PinnedStorageProfileLabelName]
	if !okName {
		return nil, dwsv1alpha2.NewResourceError("unable to find %s label", nnfv1alpha2.PinnedStorageProfileLabelName).WithFatal()
	}
	pinnedNamespace, okNamespace := labels[nnfv1alpha2.PinnedStorageProfileLabelNameSpace]
	if !okNamespace {
		return nil, dwsv1alpha2.NewResourceError("unable to find %s label", nnfv1alpha2.PinnedStorageProfileLabelNameSpace).WithFatal()
	}
