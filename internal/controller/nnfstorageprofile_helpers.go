...
func findProfileToUse(ctx context.Context, clnt client.Client, args map[string]string) (*nnfv1alpha3.NnfStorageProfile, error) {
	var profileName string

	nnfStorageProfile := &nnfv1alpha3.NnfStorageProfile{}
	...
	if present == false {
		nnfStorageProfiles := &nnfv1alpha3.NnfStorageProfileList{}
		...
		if len(profilesFound) == 0 {
			return nil, dwsv1alpha3.NewResourceError("").WithUserMessage("Unable to find a default NnfStorageProfile to use").WithFatal()
		} else if len(profilesFound) > 1 {
			return nil, dwsv1alpha3.NewResourceError("").WithUserMessage("More than one default NnfStorageProfile found; unable to pick one: %v", profilesFound).WithFatal()
		}
		profileName = profilesFound[0]
	}
	if len(profileName) == 0 {
		return nil, dwsv1alpha3.NewResourceError("").WithUserMessage("Unable to find an NnfStorageProfile name").WithUser().WithFatal()
	}
	err := clnt.Get(ctx, types.NamespacedName{Namespace: profileNamespace, Name: profileName}, nnfStorageProfile)
	if err != nil {
		return nil, dwsv1alpha3.NewResourceError("").WithUserMessage("Unable to find NnfStorageProfile: %s", profileName).WithUser().WithFatal()
	}
...
func findPinnedProfile(ctx context.Context, clnt client.Client, namespace string, pinnedName string) (*nnfv1alpha3.NnfStorageProfile, error) {
	...
	if !nnfStorageProfile.Data.Pinned {
		return nil, dwsv1alpha3.NewResourceError("Expected pinned NnfStorageProfile, but it was not pinned: %s", pinnedName).WithFatal()
	}
	return nnfStorageProfile, nil
}

// createPinnedProfile finds the specified profile and makes a pinned copy of it.
func createPinnedProfile(ctx context.Context, clnt client.Client, clntScheme *runtime.Scheme, args map[string]string, owner metav1.Object, pinnedName string) (*nnfv1alpha3.NnfStorageProfile, error) {
...
func addPinnedStorageProfileLabel(object metav1.Object, nnfStorageProfile *nnfv1alpha3.NnfStorageProfile) {
	labels := object.GetLabels()
	if labels == nil {
		labels = make(map[string]string)
	}

	labels[nnfv1alpha3.PinnedStorageProfileLabelName] = nnfStorageProfile.GetName()
	labels[nnfv1alpha3.PinnedStorageProfileLabelNameSpace] = nnfStorageProfile.GetNamespace()
	object.SetLabels(labels)
}

// getPinnedStorageProfileFromLabel finds the pinned storage profile via the labels on the
// specified resource.
func getPinnedStorageProfileFromLabel(ctx context.Context, clnt client.Client, object metav1.Object) (*nnfv1alpha3.NnfStorageProfile, error) {
	labels := object.GetLabels()
	if labels == nil {
		return nil, dwsv1alpha3.NewResourceError("unable to find labels").WithFatal()
	}

	pinnedName, okName := labels[nnfv1alpha3.PinnedStorageProfileLabelName]
	if !okName {
		return nil, dwsv1alpha3.NewResourceError("unable to find %s label", nnfv1alpha3.PinnedStorageProfileLabelName).WithFatal()
	}
	pinnedNamespace, okNamespace := labels[nnfv1alpha3.PinnedStorageProfileLabelNameSpace]
	if !okNamespace {
		return nil, dwsv1alpha3.NewResourceError("unable to find %s label", nnfv1alpha3.PinnedStorageProfileLabelNameSpace).WithFatal()
	}
