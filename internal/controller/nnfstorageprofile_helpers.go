...
func getPinnedStorageProfileFromLabel(ctx context.Context, clnt client.Client, object metav1.Object) (*nnfv1alpha4.NnfStorageProfile, error) {
	...
	if !okName {
		return nil, dwsv1alpha3.NewResourceError("unable to find %s label", nnfv1alpha4.PinnedStorageProfileLabelName).WithFatal()
	}
	pinnedNamespace, okNamespace := labels[nnfv1alpha4.PinnedStorageProfileLabelNameSpace]
	if !okNamespace {
		return nil, dwsv1alpha3.NewResourceError("unable to find %s label", nnfv1alpha4.PinnedStorageProfileLabelNameSpace).WithFatal()
	}
