...
func findProfileToUse(ctx context.Context, clnt client.Client, args map[string]string) (*nnfv1alpha10.NnfStorageProfile, error) {
	...
	if present == false {
		nnfStorageProfiles := &nnfv1alpha10.NnfStorageProfileList{}
...
func findPinnedProfile(ctx context.Context, clnt client.Client, namespace string, pinnedName string) (*nnfv1alpha10.NnfStorageProfile, error) {
...
func createPinnedProfile(ctx context.Context, clnt client.Client, clntScheme *runtime.Scheme, args map[string]string, owner metav1.Object, pinnedName string) (*nnfv1alpha10.NnfStorageProfile, error) {
...
func addPinnedStorageProfileLabel(object metav1.Object, nnfStorageProfile *nnfv1alpha10.NnfStorageProfile) {
	labels := object.GetLabels()
	if labels == nil {
		labels = make(map[string]string)
	}

	labels[nnfv1alpha10.PinnedStorageProfileLabelName] = nnfStorageProfile.GetName()
	labels[nnfv1alpha10.PinnedStorageProfileLabelNameSpace] = nnfStorageProfile.GetNamespace()
	object.SetLabels(labels)
}

// getPinnedStorageProfileFromLabel finds the pinned storage profile via the labels on the
// specified resource.
func getPinnedStorageProfileFromLabel(ctx context.Context, clnt client.Client, object metav1.Object) (*nnfv1alpha10.NnfStorageProfile, error) {
	labels := object.GetLabels()
	if labels == nil {
		return nil, dwsv1alpha7.NewResourceError("unable to find labels").WithFatal()
	}

	pinnedName, okName := labels[nnfv1alpha10.PinnedStorageProfileLabelName]
	if !okName {
		return nil, dwsv1alpha7.NewResourceError("unable to find %s label", nnfv1alpha10.PinnedStorageProfileLabelName).WithFatal()
	}
	pinnedNamespace, okNamespace := labels[nnfv1alpha10.PinnedStorageProfileLabelNameSpace]
	if !okNamespace {
		return nil, dwsv1alpha7.NewResourceError("unable to find %s label", nnfv1alpha10.PinnedStorageProfileLabelNameSpace).WithFatal()
	}
