...
func getContainerProfile(ctx context.Context, clnt client.Client, workflow *dwsv1alpha4.Workflow, index int) (*nnfv1alpha7.NnfContainerProfile, error) {
	...
	if profile == nil {
		return nil, dwsv1alpha4.NewResourceError("container profile '%s' not found", indexedResourceName(workflow, index)).WithFatal()
	}

	return profile, nil
}

func findPinnedContainerProfile(ctx context.Context, clnt client.Client, workflow *dwsv1alpha4.Workflow, index int) (*nnfv1alpha7.NnfContainerProfile, error) {
	...
	if !profile.Data.Pinned {
		return nil, dwsv1alpha4.NewResourceError("expected a pinned container profile '%s', but found one that is not pinned", indexedResourceName(workflow, index)).WithFatal()
	}

	return profile, nil
}

func findContainerProfile(ctx context.Context, clnt client.Client, workflow *dwsv1alpha4.Workflow, index int) (*nnfv1alpha7.NnfContainerProfile, error) {
	...
	if profile.Data.Pinned {
		return nil, dwsv1alpha4.NewResourceError("expected container profile that is not pinned '%s', but found one that is pinned", indexedResourceName(workflow, index)).WithFatal()
	}
	...
	if profile.Data.UserID != nil && *profile.Data.UserID != workflow.Spec.UserID {
		return nil, dwsv1alpha4.NewResourceError("").WithUserMessage(restrictedMsg, profile.Name, "UserID", *profile.Data.UserID).WithUser().WithFatal()
	}
	if profile.Data.GroupID != nil && *profile.Data.GroupID != workflow.Spec.GroupID {
		return nil, dwsv1alpha4.NewResourceError("").WithUserMessage(restrictedMsg, profile.Name, "GroupID", *profile.Data.GroupID).WithUser().WithFatal()
...
func createPinnedContainerProfileIfNecessary(ctx context.Context, clnt client.Client, scheme *kruntime.Scheme, workflow *dwsv1alpha4.Workflow, index int, log logr.Logger) error {
	profile, err := findPinnedContainerProfile(ctx, clnt, workflow, index)
	...
	dwsv1alpha4.AddOwnerLabels(pinnedProfile, workflow)
