...
//+kubebuilder:rbac:groups=nnf.cray.hpe.com,resources=nnfcontainerdata,verbs=get;list;watch;create;update;patch;delete
...
func (r *NnfWorkflowReconciler) startDataInOutState(ctx context.Context, workflow *dwsv1alpha7.Workflow, index int) (*result, error) {
	...
	prepareStagingArgumentFn := func(param string) (*corev1.ObjectReference, *nnfv1alpha11.NnfAccess, *result, error) {
	...
	// Wait for accesses to go ready
	for _, access := range []*nnfv1alpha11.NnfAccess{sourceAccess, destAccess} {
		if access != nil {
	...
	dmm := &nnfv1alpha11.NnfDataMovementManager{ObjectMeta: metav1.ObjectMeta{
		Name:      nnfv1alpha11.DataMovementManagerName,
		Namespace: nnfv1alpha11.DataMovementNamespace,
	}}
	...
	targetStorage := &nnfv1alpha11.NnfStorage{
		ObjectMeta: metav1.ObjectMeta{
		...
		for _, node := range nodes {
			...
			for i := 0; i < node.Count; i++ {
				dm := &nnfv1alpha11.NnfDataMovement{
					...
					Spec: nnfv1alpha11.NnfDataMovementSpec{
						Source: &nnfv1alpha11.NnfDataMovementSpecSourceDestination{
				...
				nnfv1alpha11.AddDataMovementTeardownStateLabel(dm, workflow.Status.State)
				nnfv1alpha11.AddDataMovementInitiatorLabel(dm, dwArgs["command"])
...
func (r *NnfWorkflowReconciler) finishDataInOutState(ctx context.Context, workflow *dwsv1alpha7.Workflow, index int) (*result, error) {
	...
	matchingLabels[nnfv1alpha11.DirectiveIndexLabel] = strconv.Itoa(index)
	matchingLabels[nnfv1alpha11.DataMovementTeardownStateLabel] = string(workflow.Status.State)
...
func (r *NnfWorkflowReconciler) startTeardownState(ctx context.Context, workflow *dwsv1alpha7.Workflow, index int) (*result, error) {
	...
	default:
		...
		childObjects := []dwsv1alpha7.ObjectList{
			&nnfv1alpha11.NnfDataMovementList{},
			&nnfv1alpha11.NnfAccessList{},
		}

		deleteStatus, err := dwsv1alpha7.DeleteChildrenWithLabels(ctx, r.Client, childObjects, workflow, client.MatchingLabels{nnfv1alpha11.DirectiveIndexLabel: strconv.Itoa(index)})
		if err != nil {
...
func (r *NnfWorkflowReconciler) finishTeardownState(ctx context.Context, workflow *dwsv1alpha7.Workflow, index int) (*result, error) {
	...
	case "create_persistent":
		...
		delete(labels, nnfv1alpha11.DirectiveIndexLabel)
...
func (r *NnfWorkflowReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Owns(&nnfv1alpha11.NnfAccess{}).
		Owns(&dwsv1alpha7.DirectiveBreakdown{}).
		Owns(&dwsv1alpha7.PersistentStorageInstance{}).
		Watches(&nnfv1alpha11.NnfDataMovement{}, handler.EnqueueRequestsFromMapFunc(dwsv1alpha7.OwnerLabelMapFunc)).
		Watches(&nnfv1alpha11.NnfStorage{}, handler.EnqueueRequestsFromMapFunc(dwsv1alpha7.OwnerLabelMapFunc)).
		Complete(r)
}
