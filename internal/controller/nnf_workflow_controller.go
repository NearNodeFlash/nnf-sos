...
func (r *NnfWorkflowReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	statusUpdater := updater.NewStatusUpdater[*dwsv1alpha4.WorkflowStatus](workflow)
	...
	if !workflow.GetDeletionTimestamp().IsZero() {
		...
		deleteStatus, err := dwsv1alpha4.DeleteChildren(ctx, r.Client, r.getChildObjects(), workflow)
		if err != nil {
			return ctrl.Result{}, err
		}
	...
	startFunctions := map[dwsv1alpha4.WorkflowState]func(*NnfWorkflowReconciler, context.Context, *dwsv1alpha4.Workflow, int) (*result, error){
		dwsv1alpha4.StateProposal: (*NnfWorkflowReconciler).startProposalState,
		dwsv1alpha4.StateSetup:    (*NnfWorkflowReconciler).startSetupState,
		dwsv1alpha4.StateDataIn:   (*NnfWorkflowReconciler).startDataInOutState,
		dwsv1alpha4.StatePreRun:   (*NnfWorkflowReconciler).startPreRunState,
		dwsv1alpha4.StatePostRun:  (*NnfWorkflowReconciler).startPostRunState,
		dwsv1alpha4.StateDataOut:  (*NnfWorkflowReconciler).startDataInOutState,
		dwsv1alpha4.StateTeardown: (*NnfWorkflowReconciler).startTeardownState,
	}
	...
	for _, driverStatus := range driverList {
		...
		driverStatus.Status = dwsv1alpha4.StatusRunning
	...
	finishFunctions := map[dwsv1alpha4.WorkflowState]func(*NnfWorkflowReconciler, context.Context, *dwsv1alpha4.Workflow, int) (*result, error){
		dwsv1alpha4.StateProposal: (*NnfWorkflowReconciler).finishProposalState,
		dwsv1alpha4.StateSetup:    (*NnfWorkflowReconciler).finishSetupState,
		dwsv1alpha4.StateDataIn:   (*NnfWorkflowReconciler).finishDataInOutState,
		dwsv1alpha4.StatePreRun:   (*NnfWorkflowReconciler).finishPreRunState,
		dwsv1alpha4.StatePostRun:  (*NnfWorkflowReconciler).finishPostRunState,
		dwsv1alpha4.StateDataOut:  (*NnfWorkflowReconciler).finishDataInOutState,
		dwsv1alpha4.StateTeardown: (*NnfWorkflowReconciler).finishTeardownState,
	}
	...
	for _, driverStatus := range driverList {
		...
		ts := metav1.NowMicro()
		driverStatus.Status = dwsv1alpha4.StatusCompleted
		driverStatus.Message = ""
		driverStatus.Error = ""
...
func (r *NnfWorkflowReconciler) startProposalState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	...
	if err := r.validateWorkflow(ctx, workflow); err != nil {
		return nil, dwsv1alpha4.NewResourceError("").WithError(err).WithUserMessage("unable to validate DW directives")
	}
	...
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("could not generate DirectiveBreakdown").WithError(err).WithUserMessage("unable to start parsing DW directive")
	}
	...
	directiveBreakdownReference := v1.ObjectReference{
		Kind:      reflect.TypeOf(dwsv1alpha4.DirectiveBreakdown{}).Name(),
...
func (r *NnfWorkflowReconciler) intepretRequiresKeyword(ctx context.Context, workflow *dwsv1alpha4.Workflow, requiresList string) error {
...
func (r *NnfWorkflowReconciler) finishProposalState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	dwArgs, _ := dwdparse.BuildArgsMap(workflow.Spec.DWDirectives[index])
	...
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("could not get DirectiveBreakdown: %v", client.ObjectKeyFromObject(directiveBreakdown)).WithError(err).WithUserMessage("unable to finish parsing DW directive")
	}
...
func (r *NnfWorkflowReconciler) startSetupState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	log := r.Log.WithValues("Workflow", client.ObjectKeyFromObject(workflow), "Index", index)
	dwArgs, _ := dwdparse.BuildArgsMap(workflow.Spec.DWDirectives[index])
	...
	case "jobdw", "create_persistent":
		...
		dbd := &dwsv1alpha4.DirectiveBreakdown{
			ObjectMeta: metav1.ObjectMeta{
				Name:      indexedResourceName(workflow, index),
		...
		if err != nil {
			return nil, dwsv1alpha4.NewResourceError("unable to get DirectiveBreakdown: %v", client.ObjectKeyFromObject(dbd)).WithError(err).WithUserMessage("could not read allocation request")
		}
		...
		if err != nil {
			return nil, dwsv1alpha4.NewResourceError("unable to get Servers: %v", client.ObjectKeyFromObject(s)).WithError(err).WithUserMessage("could not read allocation request")
		}
		...
		if _, present := os.LookupEnv("RABBIT_TEST_ENV_BYPASS_SERVER_STORAGE_CHECK"); !present {
			if err := r.validateServerAllocations(ctx, dbd, s); err != nil {
				return nil, dwsv1alpha4.NewResourceError("invalid Rabbit allocations for servers: %v", client.ObjectKeyFromObject(s)).WithError(err).WithUserMessage("invalid Rabbit allocations")
			}
		}
		...
		if storage, err := r.createNnfStorage(ctx, workflow, s, index, log); err != nil {
			...
			return nil, dwsv1alpha4.NewResourceError("could not create NnfStorage").WithError(err).WithUserMessage("could not create allocation")
		}
	case "container":
		for _, value := range workflow.Status.Requires {
...
func (r *NnfWorkflowReconciler) finishSetupState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	dwArgs, _ := dwdparse.BuildArgsMap(workflow.Spec.DWDirectives[index])
	...
	default:
		...
		if err := r.Get(ctx, client.ObjectKeyFromObject(nnfStorage), nnfStorage); err != nil {
			return nil, dwsv1alpha4.NewResourceError("could not get NnfStorage: %v", client.ObjectKeyFromObject(nnfStorage)).WithError(err).WithUserMessage("could not allocate storage")
		}
		...
		if nnfStorage.Status.Error != nil {
			handleWorkflowErrorByIndex(dwsv1alpha4.NewResourceError("storage resource error: %v", client.ObjectKeyFromObject(nnfStorage)).WithError(nnfStorage.Status.Error).WithUserMessage("could not allocate storage"), workflow, index)
...
func (r *NnfWorkflowReconciler) startDataInOutState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	log := r.Log.WithValues("Workflow", client.ObjectKeyFromObject(workflow), "Index", index)

	dwArgs, err := dwdparse.BuildArgsMap(workflow.Spec.DWDirectives[index])
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("").WithUserMessage("Invalid DW directive: %v", workflow.Spec.DWDirectives[index]).WithFatal().WithUser()
	}
	...
	prepareStagingArgumentFn := func(param string) (*corev1.ObjectReference, *nnfv1alpha7.NnfAccess, *result, error) {
		...
		if strings.HasPrefix(param, "$DW_JOB_") || strings.HasPrefix(param, "$DW_PERSISTENT_") {
			...
			if parentDwIndex < 0 {
				return nil, nil, nil, dwsv1alpha4.NewResourceError("").WithUserMessage("no directive matching '%v' found in workflow", name).WithFatal().WithUser()
			}
			...
			if err := r.Get(ctx, client.ObjectKeyFromObject(storage), storage); err != nil {
				return nil, nil, nil, dwsv1alpha4.NewResourceError("could not get NnfStorage %v", client.ObjectKeyFromObject(storage)).WithError(err).WithUserMessage("could not find storage allocation")
			}
			...
			if err != nil {
				return storageReference, access, nil, dwsv1alpha4.NewResourceError("").WithError(err).WithUserMessage("could not create data movement mount points")
			}
		...
		return nil, nil, nil, dwsv1alpha4.NewResourceError("").WithUserMessage("Staging parameter '%s' is invalid", param).WithFatal().WithUser()
	}

	sourceStorage, sourceAccess, result, err := prepareStagingArgumentFn(dwArgs["source"])
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("").WithError(err).WithUserMessage("could not prepare data movement resources")
	} else if result != nil {
		return result, nil
	}
	...
	// Wait for accesses to go ready
	for _, access := range []*nnfv1alpha7.NnfAccess{sourceAccess, destAccess} {
		if access != nil {
			if err := r.Get(ctx, client.ObjectKeyFromObject(access), access); err != nil {
				return nil, dwsv1alpha4.NewResourceError("could not get NnfAccess %v", client.ObjectKeyFromObject(access)).WithError(err).WithUserMessage("could not create data movement mount points")
			}
	...
	dmm := &nnfv1alpha7.NnfDataMovementManager{ObjectMeta: metav1.ObjectMeta{
	...
	if err := r.Get(ctx, client.ObjectKeyFromObject(dmm), dmm); err != nil {
		return nil, dwsv1alpha4.NewResourceError("could not get NnfDataMovementManager %v", client.ObjectKeyFromObject(dmm)).WithError(err).WithUserMessage("could not determine data movement readiness")
	}
	if !dmm.Status.Ready {
	...
	if workflow.Spec.DesiredState == dwsv1alpha4.StateDataIn {
		targetStorageRef = destStorage
	} else {
		targetStorageRef = sourceStorage
	}

	targetStorage := &nnfv1alpha7.NnfStorage{
		ObjectMeta: metav1.ObjectMeta{
	...
	if err := r.Get(ctx, client.ObjectKeyFromObject(targetStorage), targetStorage); err != nil {
		return nil, dwsv1alpha4.NewResourceError("could not get NnfStorage: %v", client.ObjectKeyFromObject(targetStorage)).WithError(err).WithUserMessage("could not find storage allocations")
	}
	...
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("could not get NnfDataMovementProfile %s", indexedResourceName(workflow, index)).WithError(err).WithUserMessage("could not find data movement profile")
	}
	...
	case "xfs", "gfs2":
		...
		if len(targetStorage.Spec.AllocationSets) != 1 {
			return nil, dwsv1alpha4.NewResourceError("file system %s has unexpected allocation sets %d", fsType, len(targetStorage.Spec.AllocationSets)).WithUserMessage("unexpected allocation count").WithFatal()
		}
		...
		for _, node := range nodes {
			...
			for i := 0; i < node.Count; i++ {
				...
				dwsv1alpha4.AddWorkflowLabels(dm, workflow)
				dwsv1alpha4.AddOwnerLabels(dm, workflow)
				...
				if err := r.Create(ctx, dm); err != nil {
					if !errors.IsAlreadyExists(err) {
						return nil, dwsv1alpha4.NewResourceError("could not create NnfDataMovement: %v", client.ObjectKeyFromObject(dm)).WithError(err).WithUserMessage("could not start data movement")
					}
				}
			}
...
func (r *NnfWorkflowReconciler) finishDataInOutState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {

	// Wait for data movement resources to complete

	matchingLabels := dwsv1alpha4.MatchingOwner(workflow)
	matchingLabels[nnfv1alpha7.DirectiveIndexLabel] = strconv.Itoa(index)
	matchingLabels[nnfv1alpha7.DataMovementTeardownStateLabel] = string(workflow.Status.State)

	dataMovementList := &nnfv1alpha7.NnfDataMovementList{}
	if err := r.List(ctx, dataMovementList, matchingLabels); err != nil {
		return nil, dwsv1alpha4.NewResourceError("could not list NnfDataMovements with labels: %v", matchingLabels).WithError(err).WithUserMessage("could not find data movement information")
	}
	...
	for _, dm := range dataMovementList.Items {
		if dm.Status.Status != nnfv1alpha7.DataMovementConditionReasonSuccess {
			handleWorkflowErrorByIndex(dwsv1alpha4.NewResourceError("").WithUserMessage(
...
func (r *NnfWorkflowReconciler) startPreRunState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	log := r.Log.WithValues("Workflow", client.ObjectKeyFromObject(workflow), "Index", index)
	dwArgs, _ := dwdparse.BuildArgsMap(workflow.Spec.DWDirectives[index])
	...
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("could not unmount NnfAccess index: %v", index).WithError(err).WithUserMessage("could not unmount on Rabbit nodes")
	}
	...
	if dwArgs["command"] == "container" {
		...
		if err != nil {
			return nil, dwsv1alpha4.NewResourceError("").WithError(err).WithUserMessage("unable to create/update Container Jobs: " + err.Error())
		}
	...
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("Could not CreateOrUpdate compute node NnfAccess: %v", client.ObjectKeyFromObject(access)).WithError(err).WithUserMessage("could not mount file system on compute nodes")
	}
	...
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("").WithError(err).WithFatal().WithUser().WithUserMessage("Unable to determine directive file system type")
	}
	...
	if fsType == "gfs2" || fsType == "lustre" {
		...
		if err != nil {
			return nil, dwsv1alpha4.NewResourceError("could not setup NNF Access in state %s", workflow.Status.State).WithError(err).WithUserMessage("could not mount file system on Rabbit nodes")
		}
	}
...
func (r *NnfWorkflowReconciler) finishPreRunState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	dwArgs, _ := dwdparse.BuildArgsMap(workflow.Spec.DWDirectives[index])
	...
	default:
		return nil, dwsv1alpha4.NewResourceError("unexpected directive: %v", dwArgs["command"]).WithFatal().WithUserMessage("could not mount file system on compute nodes")
	}
	...
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("could not mount rabbit NnfAccess for index %v", index).WithError(err).WithUserMessage("could not mount file system on compute nodes")
	} else if result != nil {
		return result, nil
	}
...
func (r *NnfWorkflowReconciler) startPostRunState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	...
	if result, err := r.unmountNnfAccessIfNecessary(ctx, workflow, index, "computes"); result != nil || err != nil {
		if err != nil {
			return nil, dwsv1alpha4.NewResourceError("").WithError(err).WithUserMessage("could not unmount file system from compute nodes")
		}
	...
	// Wait for data movement resources to complete
	matchingLabels := dwsv1alpha4.MatchingOwner(workflow)
	matchingLabels[nnfv1alpha7.DataMovementTeardownStateLabel] = string(dwsv1alpha4.StatePostRun)
...
func (r *NnfWorkflowReconciler) finishPostRunState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	dwArgs, _ := dwdparse.BuildArgsMap(workflow.Spec.DWDirectives[index])
	...
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("could not unmount compute NnfAccess for index %v", index).WithError(err).WithUserMessage("could not unmount file system on compute nodes")
	} else if result != nil {
		return result, nil
	}
	...
	for _, dm := range dataMovementList.Items {
		...
		if dm.Status.Status == nnfv1alpha7.DataMovementConditionReasonFailed {
			handleWorkflowErrorByIndex(dwsv1alpha4.NewResourceError("data movement %v failed", client.ObjectKeyFromObject(&dm)).WithUserMessage("data movement failed").WithFatal(), workflow, index)
			return Requeue("error").withObject(&dm), nil
		}
	}
...
func (r *NnfWorkflowReconciler) startTeardownState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	dwArgs, _ := dwdparse.BuildArgsMap(workflow.Spec.DWDirectives[index])
	...
	default:
		...
		childObjects := []dwsv1alpha4.ObjectList{
			&nnfv1alpha7.NnfDataMovementList{},
			&nnfv1alpha7.NnfAccessList{},
		}

		deleteStatus, err := dwsv1alpha4.DeleteChildrenWithLabels(ctx, r.Client, childObjects, workflow, client.MatchingLabels{nnfv1alpha7.DirectiveIndexLabel: strconv.Itoa(index)})
		if err != nil {
			return nil, dwsv1alpha4.NewResourceError("could not delete NnfDataMovement and NnfAccess children").WithError(err).WithUserMessage("could not stop data movement and unmount file systems")
		}
...
func (r *NnfWorkflowReconciler) finishTeardownState(ctx context.Context, workflow *dwsv1alpha4.Workflow, index int) (*result, error) {
	log := r.Log.WithValues("Workflow", client.ObjectKeyFromObject(workflow), "Index", index)
	dwArgs, _ := dwdparse.BuildArgsMap(workflow.Spec.DWDirectives[index])
	...
	case "create_persistent":
		for _, driverStatus := range workflow.Status.Drivers {
			if driverStatus.WatchState == dwsv1alpha4.StateTeardown {
				continue
			}
		...
		if err != nil {
			return nil, dwsv1alpha4.NewResourceError("").WithError(err).WithUserMessage("could not find persistent storage %v", dwArgs["name"])
		}

		persistentStorage.SetOwnerReferences([]metav1.OwnerReference{})
		dwsv1alpha4.RemoveOwnerLabels(persistentStorage)
		labels := persistentStorage.GetLabels()
		delete(labels, nnfv1alpha7.DirectiveIndexLabel)
		persistentStorage.SetLabels(labels)

		err = r.Update(ctx, persistentStorage)
		if err != nil {
			return nil, dwsv1alpha4.NewResourceError("could not update PersistentStorage: %v", client.ObjectKeyFromObject(persistentStorage)).WithError(err).WithUserMessage("could not finalize peristent storage")
		}
		...
		labels = persistentStorage.GetLabels()
		if labels != nil {
			if ownerUid, exists := labels[dwsv1alpha4.OwnerUidLabel]; exists {
	...
	case "destroy_persistent":
		...
		if err != nil {
			if !apierrors.IsNotFound(err) {
				return nil, dwsv1alpha4.NewResourceError("").WithError(err).WithFatal().WithUser().WithUserMessage("could not find peristent storage %v", dwArgs["name"])
			}
		...
		if persistentStorage.Spec.UserID != workflow.Spec.UserID {
			return nil, dwsv1alpha4.NewResourceError("Existing persistent storage user ID %v does not match user ID %v", persistentStorage.Spec.UserID, workflow.Spec.UserID).WithError(err).WithUserMessage("user ID does not match existing persistent storage").WithFatal().WithUser()
		}
		...
		if len(persistentStorage.Spec.ConsumerReferences) != 0 {
			...
			log.Info(err.Error())
			return nil, dwsv1alpha4.NewResourceError("persistent storage cannot be deleted with %v consumers", len(persistentStorage.Spec.ConsumerReferences)).WithError(err).WithUserMessage("persistent storage cannot be deleted while in use").WithFatal().WithUser()
		}

		persistentStorage.Spec.State = dwsv1alpha4.PSIStateDestroying

		dwsv1alpha4.AddOwnerLabels(persistentStorage, workflow)
		...
		if err := controllerutil.SetControllerReference(workflow, persistentStorage, r.Scheme); err != nil {
			return nil, dwsv1alpha4.NewResourceError("could not assign workflow as owner of PersistentInstance: %v", client.ObjectKeyFromObject(persistentStorage)).WithError(err).WithUserMessage("could not delete persistent storage %v", dwArgs["name"])
		}

		err = r.Update(ctx, persistentStorage)
		if err != nil {
			return nil, dwsv1alpha4.NewResourceError("could not update PersistentInstance: %v", client.ObjectKeyFromObject(persistentStorage)).WithError(err).WithUserMessage("could not delete persistent storage %v", dwArgs["name"])
		}
		...
		ownerUid, exists := labels[dwsv1alpha4.OwnerUidLabel]
	...
	case "persistentdw":
		...
		if err != nil {
			return nil, dwsv1alpha4.NewResourceError("").WithError(err).WithUserMessage("Could not remove persistent storage reference")
		}
	case "container":
	...
	childObjects := []dwsv1alpha4.ObjectList{
		...
		&dwsv1alpha4.PersistentStorageInstanceList{},
	}

	deleteStatus, err := dwsv1alpha4.DeleteChildrenWithLabels(ctx, r.Client, childObjects, workflow, client.MatchingLabels{nnfv1alpha7.DirectiveIndexLabel: strconv.Itoa(index)})
	if err != nil {
		return nil, dwsv1alpha4.NewResourceError("could not delete NnfStorage and PersistentStorageInstance children").WithError(err).WithUserMessage("could not delete storage allocations")
	}
...
func (r *NnfWorkflowReconciler) getChildObjects() []dwsv1alpha4.ObjectList {
	return []dwsv1alpha4.ObjectList{
		&nnfv1alpha7.NnfDataMovementList{},
		&nnfv1alpha7.NnfAccessList{},
		&nnfv1alpha7.NnfStorageList{},
		&dwsv1alpha4.PersistentStorageInstanceList{},
		&dwsv1alpha4.DirectiveBreakdownList{},
	}
}
...
func (r *NnfWorkflowReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&dwsv1alpha4.Workflow{}).
		Owns(&nnfv1alpha7.NnfAccess{}).
		Owns(&dwsv1alpha4.DirectiveBreakdown{}).
		Owns(&dwsv1alpha4.PersistentStorageInstance{}).
		Watches(&nnfv1alpha7.NnfDataMovement{}, handler.EnqueueRequestsFromMapFunc(dwsv1alpha4.OwnerLabelMapFunc)).
		Watches(&nnfv1alpha7.NnfStorage{}, handler.EnqueueRequestsFromMapFunc(dwsv1alpha4.OwnerLabelMapFunc)).
		Complete(r)
}
