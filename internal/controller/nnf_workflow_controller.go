...
func (r *NnfWorkflowReconciler) startDataInOutState(ctx context.Context, workflow *dwsv1alpha7.Workflow, index int) (*result, error) {
	...
	prepareStagingArgumentFn := func(param string) (*corev1.ObjectReference, *nnfv1alpha10.NnfAccess, *result, error) {
	...
	// Wait for accesses to go ready
	for _, access := range []*nnfv1alpha10.NnfAccess{sourceAccess, destAccess} {
		if access != nil {
	...
	dmm := &nnfv1alpha10.NnfDataMovementManager{ObjectMeta: metav1.ObjectMeta{
	...
	targetStorage := &nnfv1alpha10.NnfStorage{
		ObjectMeta: metav1.ObjectMeta{
	...
	dmProfileRef := corev1.ObjectReference{
		Kind:      reflect.TypeOf(nnfv1alpha10.NnfDataMovementProfile{}).Name(),
		Name:      dmProfile.Name,
			...
			for i := 0; i < node.Count; i++ {
				dm := &nnfv1alpha10.NnfDataMovement{
					...
					Spec: nnfv1alpha10.NnfDataMovementSpec{
						Source: &nnfv1alpha10.NnfDataMovementSpecSourceDestination{
							Path:             getRabbitRelativePath(fsType, sourceStorage, sourceAccess, source, node.Name, i),
							StorageReference: *sourceStorage,
						},
						Destination: &nnfv1alpha10.NnfDataMovementSpecSourceDestination{
...
func (r *NnfWorkflowReconciler) finishDataInOutState(ctx context.Context, workflow *dwsv1alpha7.Workflow, index int) (*result, error) {
	...
	matchingLabels[nnfv1alpha10.DirectiveIndexLabel] = strconv.Itoa(index)
	matchingLabels[nnfv1alpha10.DataMovementTeardownStateLabel] = string(workflow.Status.State)

	dataMovementList := &nnfv1alpha10.NnfDataMovementList{}
	...
	for _, dm := range dataMovementList.Items {
		if dm.Status.State != nnfv1alpha10.DataMovementConditionTypeFinished {
	...
	for _, dm := range dataMovementList.Items {
		if dm.Status.Status != nnfv1alpha10.DataMovementConditionReasonSuccess {
...
func (r *NnfWorkflowReconciler) startPostRunState(ctx context.Context, workflow *dwsv1alpha7.Workflow, index int) (*result, error) {
	...
	// Wait for data movement resources to complete
	matchingLabels := dwsv1alpha7.MatchingOwner(workflow)
	matchingLabels[nnfv1alpha10.DataMovementTeardownStateLabel] = string(dwsv1alpha7.StatePostRun)
...
func (r *NnfWorkflowReconciler) finishPostRunState(ctx context.Context, workflow *dwsv1alpha7.Workflow, index int) (*result, error) {
	...
	for _, dm := range dataMovementList.Items {
		...
		if dm.Status.Status == nnfv1alpha10.DataMovementConditionReasonFailed {
...
func (r *NnfWorkflowReconciler) startTeardownState(ctx context.Context, workflow *dwsv1alpha7.Workflow, index int) (*result, error) {
	...
	default:
		...
		childObjects := []dwsv1alpha7.ObjectList{
			&nnfv1alpha10.NnfDataMovementList{},
			&nnfv1alpha10.NnfAccessList{},
		}

		deleteStatus, err := dwsv1alpha7.DeleteChildrenWithLabels(ctx, r.Client, childObjects, workflow, client.MatchingLabels{nnfv1alpha10.DirectiveIndexLabel: strconv.Itoa(index)})
		if err != nil {
...
func (r *NnfWorkflowReconciler) finishTeardownState(ctx context.Context, workflow *dwsv1alpha7.Workflow, index int) (*result, error) {
	...
	case "create_persistent":
		...
		delete(labels, nnfv1alpha10.DirectiveIndexLabel)
...
func (r *NnfWorkflowReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Owns(&nnfv1alpha10.NnfAccess{}).
		Owns(&dwsv1alpha7.DirectiveBreakdown{}).
		Owns(&dwsv1alpha7.PersistentStorageInstance{}).
		Watches(&nnfv1alpha10.NnfDataMovement{}, handler.EnqueueRequestsFromMapFunc(dwsv1alpha7.OwnerLabelMapFunc)).
		Watches(&nnfv1alpha10.NnfStorage{}, handler.EnqueueRequestsFromMapFunc(dwsv1alpha7.OwnerLabelMapFunc)).
		Complete(r)
}
