...
func (r *NnfWorkflowReconciler) startPostRunState(ctx context.Context, workflow *dwsv1alpha2.Workflow, index int) (*result, error) {
	...
	// Wait for data movement resources to complete
	matchingLabels := dwsv1alpha2.MatchingOwner(workflow)
	matchingLabels[nnfv1alpha4.DataMovementTeardownStateLabel] = string(dwsv1alpha2.StatePostRun)
...
func (r *NnfWorkflowReconciler) startTeardownState(ctx context.Context, workflow *dwsv1alpha2.Workflow, index int) (*result, error) {
	...
	default:
		...
		deleteStatus, err := dwsv1alpha2.DeleteChildrenWithLabels(ctx, r.Client, childObjects, workflow, client.MatchingLabels{nnfv1alpha4.DirectiveIndexLabel: strconv.Itoa(index)})
		if err != nil {
...
func (r *NnfWorkflowReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&nnfv1alpha4.NnfDataMovement{}, handler.EnqueueRequestsFromMapFunc(dwsv1alpha2.OwnerLabelMapFunc)).
		Watches(&nnfv1alpha4.NnfStorage{}, handler.EnqueueRequestsFromMapFunc(dwsv1alpha2.OwnerLabelMapFunc)).
		Complete(r)
}
