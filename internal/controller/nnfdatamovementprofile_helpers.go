...
func findDMProfileToUse(ctx context.Context, clnt client.Client, args map[string]string) (*nnfv1alpha7.NnfDataMovementProfile, error) {
	...
	if present == false {
		...
		if len(profilesFound) == 0 {
			return nil, dwsv1alpha5.NewResourceError("").WithUserMessage("Unable to find a default NnfDataMovementProfile to use").WithFatal()
		} else if len(profilesFound) > 1 {
			return nil, dwsv1alpha5.NewResourceError("").WithUserMessage("More than one default NnfDataMovementProfile found; unable to pick one: %v", profilesFound).WithFatal()
		}
		profileName = profilesFound[0]
	}
	if len(profileName) == 0 {
		return nil, dwsv1alpha5.NewResourceError("").WithUserMessage("Unable to find an NnfDataMovementProfile name").WithUser().WithFatal()
	}
	err := clnt.Get(ctx, types.NamespacedName{Namespace: profileNamespace, Name: profileName}, NnfDataMovementProfile)
	if err != nil {
		return nil, dwsv1alpha5.NewResourceError("").WithUserMessage("Unable to find NnfDataMovementProfile: %s", profileName).WithUser().WithFatal()
	}
...
func findPinnedDMProfile(ctx context.Context, clnt client.Client, namespace string, pinnedName string) (*nnfv1alpha7.NnfDataMovementProfile, error) {
	...
	if !NnfDataMovementProfile.Data.Pinned {
		return nil, dwsv1alpha5.NewResourceError("Expected pinned NnfDataMovementProfile, but it was not pinned: %s", pinnedName).WithFatal()
	}
	return NnfDataMovementProfile, nil
}
