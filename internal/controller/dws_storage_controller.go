...
func (r *DWSStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	statusUpdater := updater.NewStatusUpdater[*dwsv1alpha7.StorageStatus](storage)
	...
	if storage.Spec.State == dwsv1alpha7.DisabledState {
	...
	storage.Status.Type = dwsv1alpha7.NVMe
	storage.Status.Capacity = nnfNode.Status.Capacity
	storage.Status.Access.Protocol = dwsv1alpha7.PCIe
	...
	storage.Status.Access.Servers = []dwsv1alpha7.Node{{
		Name:   storage.Name,
	...
	if len(nnfNode.Status.Servers) > 1 {
		storage.Status.Access.Computes = make([]dwsv1alpha7.Node, 0)
	...
	storage.Status.Devices = make([]dwsv1alpha7.StorageDevice, len(nnfNode.Status.Drives))
	...
	if storage.Status.Status == dwsv1alpha7.DisabledStatus {
	...
	if nnfNode.Status.Fenced {
		storage.Status.Status = dwsv1alpha7.DegradedStatus
		...
		if !nodeState.nodeReady {
			...
			storage.Status.Status = dwsv1alpha7.OfflineStatus
			...
			storage.Status.Status = dwsv1alpha7.DrainedStatus
...
func (r *DWSStorageReconciler) coreNodeState(ctx context.Context, storage *dwsv1alpha7.Storage) (K8sNodeState, error) {
