...
var _ = Context("NNF Port Manager Controller Setup", Ordered, func() {
	...
	Describe("NNF Port Manager Controller Test", func() {
		...
		var mgr *nnfv1alpha4.NnfPortManager
		portCooldown := 1
		...
		allocatePorts := func(mgr *nnfv1alpha4.NnfPortManager, name string, count int) []uint16 {
			By(fmt.Sprintf("Reserving %d ports for '%s'", count, name))

			allocation := nnfv1alpha4.NnfPortManagerAllocationSpec{
		...
		reservePorts := func(mgr *nnfv1alpha4.NnfPortManager, name string, count int) []uint16 {
			...
			Expect(status.Status).To(Equal(nnfv1alpha4.NnfPortManagerAllocationStatusInUse))

			return ports
		}

		reservePortsAllowFail := func(mgr *nnfv1alpha4.NnfPortManager, name string, count int) []uint16 {
			return allocatePorts(mgr, name, count)
		}

		releasePorts := func(mgr *nnfv1alpha4.NnfPortManager, name string) {
		...
		// of the test.
		kickPortManager := func(mgr *nnfv1alpha4.NnfPortManager) {
		...
		verifyNumAllocations := func(mgr *nnfv1alpha4.NnfPortManager, status nnfv1alpha4.NnfPortManagerAllocationStatusStatus, count int) {
		...
		verifyNumAllocationsInUse := func(mgr *nnfv1alpha4.NnfPortManager, count int) {
			verifyNumAllocations(mgr, nnfv1alpha4.NnfPortManagerAllocationStatusInUse, count)
		}

		verifyNumAllocationsCooldown := func(mgr *nnfv1alpha4.NnfPortManager, count int) {
			verifyNumAllocations(mgr, nnfv1alpha4.NnfPortManagerAllocationStatusCooldown, count)
		}

		verifyNumAllocationsInsuffientResources := func(mgr *nnfv1alpha4.NnfPortManager, count int) {
			verifyNumAllocations(mgr, nnfv1alpha4.NnfPortManagerAllocationStatusInsufficientResources, count)
		}
		...
		When("the system configuration is missing", func() {
			It("should have a status that indicates system configuration is not found", func() {
				...
				Eventually(func() nnfv1alpha4.NnfPortManagerStatusStatus {
					k8sClient.Get(ctx, client.ObjectKeyFromObject(mgr), mgr)
					return mgr.Status.Status
				}).Should(Equal(nnfv1alpha4.NnfPortManagerStatusSystemConfigurationNotFound))
			})
		})
		...
		When("reserving ports with portCooldown", func() {
			...
			When("all ports are already reserved", func() {
				It("fails with insufficient resources", func() {
					...
					Expect(status.Status).To(Equal(nnfv1alpha4.NnfPortManagerAllocationStatusInsufficientResources))
				})
			})
			...
			When("all ports are already reserved and another allocation is requested", func() {
				It("should eventually free up the cooldown ports and successfully reserve", func() {
					...
					allocation := nnfv1alpha4.NnfPortManagerAllocationSpec{Requester: corev1.ObjectReference{Name: "waiting"}, Count: 1}
					status := r.findAllocationStatus(mgr, allocation)
