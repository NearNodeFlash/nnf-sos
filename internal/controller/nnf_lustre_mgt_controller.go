...
func (r *NnfLustreMGTReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	statusUpdater := updater.NewStatusUpdater[*nnfv1alpha2.NnfLustreMGTStatus](nnfLustreMgt)
...
func isFsNameBlackListed(nnfLustreMgt *nnfv1alpha2.NnfLustreMGT, fsname string) bool {
	// Check the blacklist
...
func (r *NnfLustreMGTReconciler) HandleNewClaims(ctx context.Context, nnfLustreMgt *nnfv1alpha2.NnfLustreMGT) (*ctrl.Result, error) {
	...
	for _, reference := range nnfLustreMgt.Spec.ClaimList {
		if _, exists := claimMap[reference]; !exists {
			...
			newClaim := nnfv1alpha2.NnfLustreMGTStatusClaim{
				Reference: reference,
				FsName:    fsnameNext,
			}
