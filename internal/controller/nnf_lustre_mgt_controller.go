...
func (r *NnfLustreMGTReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	statusUpdater := updater.NewStatusUpdater[*nnfv1alpha8.NnfLustreMGTStatus](nnfLustreMgt)
	...
	if nnfLustreMgt.Status.FsNameNext == "" {
		...
		if nnfLustreMgt.Spec.FsNameStartReference != (corev1.ObjectReference{}) {
			if nnfLustreMgt.Spec.FsNameStartReference.Kind != reflect.TypeOf(corev1.ConfigMap{}).Name() {
				return ctrl.Result{}, dwsv1alpha5.NewResourceError("lustre MGT start reference does not have kind '%s'", reflect.TypeOf(corev1.ConfigMap{}).Name()).WithFatal().WithUser()
			}
			...
			if err := r.Get(ctx, client.ObjectKeyFromObject(configMap), configMap); err != nil {
				return ctrl.Result{}, dwsv1alpha5.NewResourceError("could not get Lustre MGT start fsname config map: %v", client.ObjectKeyFromObject(configMap)).WithError(err).WithMajor()
			}

			if configMap.Data != nil {
				if _, exists := configMap.Data["NextFsName"]; exists {
					if len(configMap.Data["NextFsName"]) != 8 {
						return ctrl.Result{}, dwsv1alpha5.NewResourceError("starting fsname from config map: %v was not 8 characters", client.ObjectKeyFromObject(configMap)).WithError(err).WithFatal()
					}
...
func isFsNameBlackListed(nnfLustreMgt *nnfv1alpha8.NnfLustreMGT, fsname string) bool {
	// Check the blacklist
...
func (r *NnfLustreMGTReconciler) SetFsNameNext(ctx context.Context, nnfLustreMgt *nnfv1alpha8.NnfLustreMGT, fsname string) (*ctrl.Result, error) {
	...
	if nnfLustreMgt.Spec.FsNameStartReference != (corev1.ObjectReference{}) {
		if nnfLustreMgt.Spec.FsNameStartReference.Kind != reflect.TypeOf(corev1.ConfigMap{}).Name() {
			return nil, dwsv1alpha5.NewResourceError("lustre MGT start reference does not have kind '%s'", reflect.TypeOf(corev1.ConfigMap{}).Name()).WithFatal().WithUser()
		}
		...
		if err := r.Get(ctx, client.ObjectKeyFromObject(configMap), configMap); err != nil {
			return nil, dwsv1alpha5.NewResourceError("could not get Lustre MGT start fsname config map: %v", client.ObjectKeyFromObject(configMap)).WithError(err).WithMajor()
		}
		...
		if err := r.Update(ctx, configMap); err != nil {
			...
			return nil, dwsv1alpha5.NewResourceError("could not update Lustre MGT used fsname config map:  %v", client.ObjectKeyFromObject(configMap)).WithError(err).WithMajor()
		}
	}
...
// an fsname
func (r *NnfLustreMGTReconciler) HandleNewClaims(ctx context.Context, nnfLustreMgt *nnfv1alpha8.NnfLustreMGT) (*ctrl.Result, error) {
	...
	for _, reference := range nnfLustreMgt.Spec.ClaimList {
		if _, exists := claimMap[reference]; !exists {
			...
			newClaim := nnfv1alpha8.NnfLustreMGTStatusClaim{
				Reference: reference,
				FsName:    fsnameNext,
			}
...
func (r *NnfLustreMGTReconciler) RemoveOldClaims(ctx context.Context, nnfLustreMgt *nnfv1alpha8.NnfLustreMGT) error {
...
func (r *NnfLustreMGTReconciler) EraseOldFsName(nnfLustreMgt *nnfv1alpha8.NnfLustreMGT, fsname string) error {
	...
	if r.ControllerType == ControllerRabbit {
		if cmd, found := os.LookupEnv("NNF_LUSTRE_FSNAME_ERASE_COMMAND"); found {
			...
			if _, err := command.Run(fmt.Sprintf("%s %s", cmd, fsname), log); err != nil {
				...
				return dwsv1alpha5.NewResourceError("unable to remove fsname '%s' from MGT", fsname).WithError(err).WithMajor()
			}
		}
	}
...
func (r *NnfLustreMGTReconciler) HandleNewCommands(ctx context.Context, nnfLustreMgt *nnfv1alpha8.NnfLustreMGT) (*ctrl.Result, error) {
	commandMap := map[corev1.ObjectReference]*nnfv1alpha8.NnfLustreMGTStatusCommand{}
	...
	for _, command := range nnfLustreMgt.Spec.CommandList {
		if commandStatus, exists := commandMap[command.Reference]; !exists || !commandStatus.Ready {
			...
			if !exists {
				commandStatus = &nnfv1alpha8.NnfLustreMGTStatusCommand{
...
func (r *NnfLustreMGTReconciler) RemoveOldCommands(ctx context.Context, nnfLustreMgt *nnfv1alpha8.NnfLustreMGT) error {
...
func (r *NnfLustreMGTReconciler) RunCommands(nnfLustreMgt *nnfv1alpha8.NnfLustreMGT, commandLines []string) error {
	log := r.Log.WithValues("NnfLustreMGT", client.ObjectKeyFromObject(nnfLustreMgt))
	if r.ControllerType != ControllerRabbit {
		return nil
	}
	...
	for _, commandLine := range commandLines {
		if _, err := command.Run(commandLine, log); err != nil {
			return dwsv1alpha5.NewResourceError("unable to run MGT command: %s", commandLine).WithError(err).WithMajor()
		}
	}
