...
func (r *NnfSystemConfigurationReconciler) createNamespaces(ctx context.Context, config *dwsv1alpha4.SystemConfiguration, validNamespaces map[string]struct{}) error {
	for name := range validNamespaces {
		...
		_, err := ctrl.CreateOrUpdate(ctx, r.Client, namespace,
			func() error {
				dwsv1alpha4.AddOwnerLabels(namespace, config)
				return nil
			})
		if err != nil {
			return err
...
func (r *NnfSystemConfigurationReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	builder := ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&corev1.Namespace{}, handler.EnqueueRequestsFromMapFunc(dwsv1alpha4.OwnerLabelMapFunc)).
