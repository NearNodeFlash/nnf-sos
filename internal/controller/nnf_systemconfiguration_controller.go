...
func (r *NnfSystemConfigurationReconciler) labelsAndTaints(ctx context.Context, validRabbits map[string]struct{}) (bool, error) {
	...
	for _, effect := range taintEffectPerPass {
		...
		for name := range validRabbits {
			...
			staleLabel := false
			_, hasCompletedLabel := labels[nnfv1alpha11.TaintsAndLabelsCompletedLabel]
			if effect == corev1.TaintEffectNoSchedule && hasCompletedLabel {
				...
				delete(labels, nnfv1alpha11.TaintsAndLabelsCompletedLabel)
			...
			if effect == clearNoExecute {
				...
				labels[nnfv1alpha11.TaintsAndLabelsCompletedLabel] = "true"
				doUpdate = true
				node.SetLabels(labels)
			} else {
				// Add the taint.
			...
			// Add the label.
			if _, present := labels[nnfv1alpha11.RabbitNodeSelectorLabel]; !present {
				labels[nnfv1alpha11.RabbitNodeSelectorLabel] = "true"
				doUpdate = true
				node.SetLabels(labels)
			}
