...
func (r *NnfWorkflowReconciler) validatePersistentInstanceForStaging(ctx context.Context, name string, namespace string) error {
	...
	if err != nil {
		return dwsv1alpha5.NewResourceError("").WithError(err).WithUserMessage("PersistentStorageInstance '%s' not found", name).WithFatal().WithUser()
	}
...
func (r *NnfWorkflowReconciler) validatePersistentInstanceDirective(ctx context.Context, wf *dwsv1alpha5.Workflow, directive string) error {
	...
	if err != nil {
		return dwsv1alpha5.NewResourceError("").WithError(err).WithUserMessage("PersistentStorageInstance '%s' not found", args["name"]).WithFatal().WithUser()
	}
