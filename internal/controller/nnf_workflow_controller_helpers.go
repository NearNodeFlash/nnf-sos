...
func (r *NnfWorkflowReconciler) findMockDevicesForKind(ctx context.Context, workflow *dwsv1alpha3.Workflow) ([]nnfContainerVolume, error) {
	volumes := []nnfContainerVolume{}

	nodeStoragesList, err := r.getNnfNodeStorages(ctx, workflow)
	if err != nil {
		return nil, dwsv1alpha3.NewResourceError("could not find devices for KIND environment").WithError(err)
	}
