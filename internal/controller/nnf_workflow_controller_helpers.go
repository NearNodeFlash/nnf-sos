...
func (r *NnfWorkflowReconciler) createNnfStorage(ctx context.Context, workflow *dwsv1alpha3.Workflow, s *dwsv1alpha3.Servers, index int, log logr.Logger) (*nnfv1alpha4.NnfStorage, error) {
	...
	result, err := ctrl.CreateOrUpdate(ctx, r.Client, nnfStorage,
		func() error {
			...
			nnfStorage.Spec.AllocationSets = []nnfv1alpha4.NnfStorageAllocationSetSpec{}

			// Iterate the Servers data elements to pull out the allocation sets for the server
			for i := range s.Spec.AllocationSets {
				nnfAllocSet := nnfv1alpha4.NnfStorageAllocationSetSpec{}
				...
				for k, storage := range s.Spec.AllocationSets[i].Storage {
					if s.Spec.AllocationSets[i].Label == "mgtmdt" && k == 0 && storage.AllocationCount > 1 {
						...
						node := nnfv1alpha4.NnfStorageAllocationNodes{Name: storage.Name, Count: 1}
						nnfAllocSet.Nodes = append(nnfAllocSet.Nodes, node)
						node = nnfv1alpha4.NnfStorageAllocationNodes{Name: storage.Name, Count: storage.AllocationCount - 1}
						nnfAllocSet.Nodes = append(nnfAllocSet.Nodes, node)
					} else {
...
func (r *NnfWorkflowReconciler) getLustreMgsFromPool(ctx context.Context, pool string) (corev1.ObjectReference, string, error) {
	...
	if err := r.List(ctx, persistentStorageList, client.MatchingLabels(map[string]string{nnfv1alpha4.StandaloneMGTLabel: pool})); err != nil {
...
func (r *NnfWorkflowReconciler) setupNnfAccessForServers(ctx context.Context, storage *nnfv1alpha4.NnfStorage, workflow *dwsv1alpha3.Workflow, index int, parentDwIndex int, teardownState dwsv1alpha3.WorkflowState, log logr.Logger) (*nnfv1alpha4.NnfAccess, error) {
	...
	result, err := ctrl.CreateOrUpdate(ctx, r.Client, access,
		func() error {
			...
			nnfv1alpha4.AddDataMovementTeardownStateLabel(access, teardownState)

			access.Spec = nnfv1alpha4.NnfAccessSpec{
				...
				StorageReference: corev1.ObjectReference{
					Kind:      reflect.TypeOf(nnfv1alpha4.NnfStorage{}).Name(),
					Name:      storage.Name,
...
func getRabbitRelativePath(fsType string, storageRef *corev1.ObjectReference, access *nnfv1alpha4.NnfAccess, path, namespace string, index int) string {
	relPath := path

	if storageRef.Kind == reflect.TypeOf(nnfv1alpha4.NnfStorage{}).Name() {
		switch fsType {
		case "xfs", "gfs2":
...
func addDirectiveIndexLabel(object metav1.Object, index int) {
	...
	labels[nnfv1alpha4.DirectiveIndexLabel] = strconv.Itoa(index)
...
func getDirectiveIndexLabel(object metav1.Object) string {
	...
	return labels[nnfv1alpha4.DirectiveIndexLabel]
}
...
func setTargetOwnerUIDLabel(object metav1.Object, value string) {
	...
	labels[nnfv1alpha4.TargetOwnerUidLabel] = value
	object.SetLabels(labels)
}
...
func getTargetOwnerUIDLabel(object metav1.Object) string {
	...
	return labels[nnfv1alpha4.TargetOwnerUidLabel]
}
...
func setTargetDirectiveIndexLabel(object metav1.Object, value string) {
	...
	labels[nnfv1alpha4.TargetDirectiveIndexLabel] = value
	object.SetLabels(labels)
}
...
func getTargetDirectiveIndexLabel(object metav1.Object) string {
	...
	return labels[nnfv1alpha4.TargetDirectiveIndexLabel]
}
...
func (r *NnfWorkflowReconciler) unmountNnfAccessIfNecessary(ctx context.Context, workflow *dwsv1alpha3.Workflow, index int, accessSuffix string) (*result, error) {
	...
	teardownState, found := access.Labels[nnfv1alpha4.DataMovementTeardownStateLabel]
...
func (r *NnfWorkflowReconciler) userContainerHandler(ctx context.Context, workflow *dwsv1alpha3.Workflow, dwArgs map[string]string, index int, log logr.Logger) (*result, error) {
	...
	c := nnfUserContainer{
		...
		nnfNodes: nnfNodes,
		volumes:  volumes,
		username: nnfv1alpha4.ContainerUser,
...
func (r *NnfWorkflowReconciler) createContainerService(ctx context.Context, workflow *dwsv1alpha3.Workflow) error {
	...
	service.Spec.Selector = map[string]string{
		nnfv1alpha4.ContainerLabel: workflow.Name,
	}
...
func (r *NnfWorkflowReconciler) deleteContainers(ctx context.Context, workflow *dwsv1alpha3.Workflow, index int) (*result, error) {
	...
	if index >= 0 {
		matchLabels[nnfv1alpha4.DirectiveIndexLabel] = strconv.Itoa(index)
	}
...
func (r *NnfWorkflowReconciler) getContainerVolumes(ctx context.Context, workflow *dwsv1alpha3.Workflow, dwArgs map[string]string, profile *nnfv1alpha4.NnfContainerProfile) ([]nnfContainerVolume, *result, error) {
	...
	for arg, val := range dwArgs {
		...
		if cmd == "globaldw" {
			...
			nnfAccess := &nnfv1alpha4.NnfAccess{
				ObjectMeta: metav1.ObjectMeta{
...
func (r *NnfWorkflowReconciler) getContainerPorts(ctx context.Context, workflow *dwsv1alpha3.Workflow, index int) (*result, error) {
	...
	if profile.Data.NumPorts > 0 {
		...
		// container profile
		pm.Spec.Allocations = append(pm.Spec.Allocations, nnfv1alpha4.NnfPortManagerAllocationSpec{
	...
	if profile.Data.NumPorts > 0 {
		...
		for _, alloc := range pm.Status.Allocations {
			if alloc.Requester != nil && alloc.Requester.UID == workflow.UID {
				if alloc.Status == nnfv1alpha4.NnfPortManagerAllocationStatusInUse && len(alloc.Ports) == int(profile.Data.NumPorts) {
					// Add workflow env var for the ports
					name, val := getContainerPortsEnvVar(alloc.Ports)
					workflow.Status.Env[name] = val
					return nil, nil // done
				} else if alloc.Status == nnfv1alpha4.NnfPortManagerAllocationStatusInvalidConfiguration {
				...
				} else if alloc.Status == nnfv1alpha4.NnfPortManagerAllocationStatusInsufficientResources {
...
func getContainerPortManager(ctx context.Context, cl client.Client) (*nnfv1alpha4.NnfPortManager, error) {
	portManagerName := os.Getenv("NNF_PORT_MANAGER_NAME")
	portManagerNamespace := os.Getenv("NNF_PORT_MANAGER_NAMESPACE")

	pm := &nnfv1alpha4.NnfPortManager{
		ObjectMeta: metav1.ObjectMeta{
...
func (r *NnfWorkflowReconciler) releaseContainerPorts(ctx context.Context, workflow *dwsv1alpha3.Workflow) (*result, error) {
	...
	for _, alloc := range pm.Status.Allocations {
		if alloc.Requester.UID == workflow.UID && alloc.Status == nnfv1alpha4.NnfPortManagerAllocationStatusInUse {
			found = true
			break
		}
	}
