...
func (r *NnfNodeECDataReconciler) Start(ctx context.Context) error {
	...
	if !testing {
		...
		data := nnfv1alpha9.NnfNodeECData{}
...
Retry:
	...
	if data.Status.Data == nil {
		data.Status.Data = make(map[string]nnfv1alpha9.NnfNodeECPrivateData)
	}

	if _, found := data.Status.Data[psi.name]; !found {
		data.Status.Data[psi.name] = make(nnfv1alpha9.NnfNodeECPrivateData)
	}
