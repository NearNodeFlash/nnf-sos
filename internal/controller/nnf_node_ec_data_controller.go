...
func (r *NnfNodeECDataReconciler) Start(ctx context.Context) error {
	...
	if !testing {
		...
		data := nnfv1alpha11.NnfNodeECData{}
...
Retry:
	...
	if data.Status.Data == nil {
		data.Status.Data = make(map[string]nnfv1alpha11.NnfNodeECPrivateData)
	}

	if _, found := data.Status.Data[psi.name]; !found {
		data.Status.Data[psi.name] = make(nnfv1alpha11.NnfNodeECPrivateData)
	}
...
func (r *NnfNodeECDataReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&nnfv1alpha11.NnfNodeECData{}).
		Complete(r)
}
