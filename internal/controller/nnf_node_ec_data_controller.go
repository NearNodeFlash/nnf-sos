...
func (r *NnfNodeECDataReconciler) Start(ctx context.Context) error {
	...
	if !testing {
		...
		data := nnfv1alpha10.NnfNodeECData{}
...
Retry:
	...
	if data.Status.Data == nil {
		data.Status.Data = make(map[string]nnfv1alpha10.NnfNodeECPrivateData)
	}

	if _, found := data.Status.Data[psi.name]; !found {
		data.Status.Data[psi.name] = make(nnfv1alpha10.NnfNodeECPrivateData)
	}
...
func (r *NnfNodeECDataReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&nnfv1alpha10.NnfNodeECData{}).
		Complete(r)
}
