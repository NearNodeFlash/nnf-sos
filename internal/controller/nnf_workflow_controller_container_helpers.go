...
func (c *nnfUserContainer) createMPIJob() error {
	...
	c.username = nnfv1alpha2.ContainerMPIUser
...
func (c *nnfUserContainer) applyLabels(job metav1.Object) error {
	...
	labels[nnfv1alpha2.ContainerLabel] = c.workflow.Name
	labels[nnfv1alpha2.PinnedContainerProfileLabelName] = c.profile.GetName()
	labels[nnfv1alpha2.PinnedContainerProfileLabelNameSpace] = c.profile.GetNamespace()
	labels[nnfv1alpha2.DirectiveIndexLabel] = strconv.Itoa(c.index)
...
func (c *nnfUserContainer) getHostPorts() ([]uint16, error) {
	...
	for _, alloc := range pm.Status.Allocations {
		if alloc.Requester != nil && alloc.Requester.UID == c.workflow.UID && alloc.Status == nnfv1alpha2.NnfPortManagerAllocationStatusInUse {
