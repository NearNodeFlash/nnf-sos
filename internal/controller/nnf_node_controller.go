	...
	if !testing {
		...
		if err := r.Get(ctx, r.NamespacedName, node); err != nil {
			...
			err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
				...
				if node.Status.Status != nnfv1alpha11.ResourceStarting {
					node.Status.Status = nnfv1alpha11.ResourceStarting
...
func (r *NnfNodeReconciler) EventHandler(e nnfevent.Event) error {
	...
	r.Events <- event.GenericEvent{Object: &nnfv1alpha11.NnfNode{
...
func (r *NnfNodeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	statusUpdater := updater.NewStatusUpdater[*nnfv1alpha11.NnfNodeStatus](node)
	...
	node.Status.Status = nnfv1alpha11.ResourceStatus(storageService.Status)
	node.Status.Health = nnfv1alpha11.ResourceHealth(storageService.Status)
...
func (r *NnfNodeReconciler) createNode() *nnfv1alpha11.NnfNode {
	return &nnfv1alpha11.NnfNode{
		ObjectMeta: metav1.ObjectMeta{
			Name:      r.Name,
		...
		Status: nnfv1alpha11.NnfNodeStatus{
			Status:   nnfv1alpha11.ResourceStarting,
			Capacity: 0,
		},
	}
}
...
func (r *NnfNodeReconciler) updateServers(node *nnfv1alpha11.NnfNode, log logr.Logger) error {
	...
	if len(node.Status.Servers) < len(serverEndpointCollection.Members) {
		node.Status.Servers = make([]nnfv1alpha11.NnfServerStatus, len(serverEndpointCollection.Members))
	}
	...
	for idx, serverEndpoint := range serverEndpointCollection.Members {
		...
		node.Status.Servers[idx].NnfResourceStatus = nnfv1alpha11.NnfResourceStatus{
			ID:     serverEndpoint.Id,
			Name:   serverEndpoint.Name,
			Status: nnfv1alpha11.ResourceStatus(serverEndpoint.Status),
			Health: nnfv1alpha11.ResourceHealth(serverEndpoint.Status),
		}
	}
...
func (r *NnfNodeReconciler) checkFencedStatus(ctx context.Context, node *nnfv1alpha11.NnfNode, log logr.Logger) error {
	...
	for i := range node.Status.Servers {
		...
		if fencedComputes[server.Hostname] {
			...
			if server.Status != nnfv1alpha11.ResourceFenced {
				log.Info("Compute node fenced", "compute", server.Hostname)
				node.Status.Servers[i].Status = nnfv1alpha11.ResourceFenced
				node.Status.Servers[i].Health = nnfv1alpha11.ResourceCritical
			}
		} else {
			// No fence file - clear fenced status if previously set
			if server.Status == nnfv1alpha11.ResourceFenced && server.Hostname != "" {
				log.Info("Compute node fence status cleared", "compute", server.Hostname)
				node.Status.Servers[i].Status = nnfv1alpha11.ResourceReady
			}
		}
	}
...
func updateDrives(node *nnfv1alpha11.NnfNode, log logr.Logger) error {
	...
	if len(node.Status.Drives) < len(storageCollection.Members) {
		node.Status.Drives = make([]nnfv1alpha11.NnfDriveStatus, len(storageCollection.Members))
	}
	...
	for idx, storageEndpoint := range storageCollection.Members {
		...
		drive.NnfResourceStatus = nnfv1alpha11.NnfResourceStatus{
			ID:     storage.Id,
			Name:   storage.Name,
			Status: nnfv1alpha11.ResourceStatus(storage.Status),
			Health: nnfv1alpha11.ResourceHealth(storage.Status),
		}
...
func (r *NnfNodeReconciler) triggerBlockStorageReconciles(ctx context.Context, computeNode string, log logr.Logger) int {
	...
	blockStorageList := &nnfv1alpha11.NnfNodeBlockStorageList{}
	...
	for _, nnfNodeBlockStorage := range blockStorageList.Items {
		...
		if hasAccess {
			...
			r.blockStorageEvents <- event.GenericEvent{
				Object: &nnfv1alpha11.NnfNodeBlockStorage{
					ObjectMeta: metav1.ObjectMeta{
