	...
	if !testing {
		...
		if err := r.Get(ctx, r.NamespacedName, node); err != nil {
			...
			err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
				...
				if node.Status.Status != nnfv1alpha6.ResourceStarting {
					node.Status.Status = nnfv1alpha6.ResourceStarting
...
func (r *NnfNodeReconciler) EventHandler(e nnfevent.Event) error {
	...
	r.Events <- event.GenericEvent{Object: &nnfv1alpha6.NnfNode{
...
func (r *NnfNodeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	statusUpdater := updater.NewStatusUpdater[*nnfv1alpha6.NnfNodeStatus](node)
	...
	node.Status.Status = nnfv1alpha6.ResourceStatus(storageService.Status)
	node.Status.Health = nnfv1alpha6.ResourceHealth(storageService.Status)
...
func (r *NnfNodeReconciler) createNode() *nnfv1alpha6.NnfNode {
	return &nnfv1alpha6.NnfNode{
		ObjectMeta: metav1.ObjectMeta{
			Name:      r.Name,
		...
		Status: nnfv1alpha6.NnfNodeStatus{
			Status:   nnfv1alpha6.ResourceStarting,
			Capacity: 0,
		},
	}
}
...
func (r *NnfNodeReconciler) updateServers(node *nnfv1alpha6.NnfNode, log logr.Logger) error {
	...
	if len(node.Status.Servers) < len(serverEndpointCollection.Members) {
		node.Status.Servers = make([]nnfv1alpha6.NnfServerStatus, len(serverEndpointCollection.Members))
	}
	...
	for idx, serverEndpoint := range serverEndpointCollection.Members {
		...
		node.Status.Servers[idx].NnfResourceStatus = nnfv1alpha6.NnfResourceStatus{
			ID:     serverEndpoint.Id,
			Name:   serverEndpoint.Name,
			Status: nnfv1alpha6.ResourceStatus(serverEndpoint.Status),
			Health: nnfv1alpha6.ResourceHealth(serverEndpoint.Status),
		}
	}
...
func updateDrives(node *nnfv1alpha6.NnfNode, log logr.Logger) error {
	...
	if len(node.Status.Drives) < len(storageCollection.Members) {
		node.Status.Drives = make([]nnfv1alpha6.NnfDriveStatus, len(storageCollection.Members))
	}
	...
	for idx, storageEndpoint := range storageCollection.Members {
		...
		drive.NnfResourceStatus = nnfv1alpha6.NnfResourceStatus{
			ID:     storage.Id,
			Name:   storage.Name,
			Status: nnfv1alpha6.ResourceStatus(storage.Status),
			Health: nnfv1alpha6.ResourceHealth(storage.Status),
		}
