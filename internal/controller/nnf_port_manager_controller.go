...
func (r *NnfPortManagerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	mgr := &nnfv1alpha2.NnfPortManager{}
	...
	statusUpdater := updater.NewStatusUpdater[*nnfv1alpha2.NnfPortManagerStatus](mgr)
	...
	mgr.Status.Status = nnfv1alpha2.NnfPortManagerStatusReady
	if err := r.Get(ctx, client.ObjectKeyFromObject(config), config); err != nil {
		...
		mgr.Status.Status = nnfv1alpha2.NnfPortManagerStatusSystemConfigurationNotFound
	...
	for _, spec := range mgr.Spec.Allocations {
		var ports []uint16
		var status nnfv1alpha2.NnfPortManagerAllocationStatusStatus
		var allocationStatus *nnfv1alpha2.NnfPortManagerAllocationStatus
		...
		if allocationStatus != nil && allocationStatus.Status == nnfv1alpha2.NnfPortManagerAllocationStatusInUse {
			continue
		}

		// Determine if the port manager is ready and find a free port
		if mgr.Status.Status != nnfv1alpha2.NnfPortManagerStatusReady {
			ports, status = nil, nnfv1alpha2.NnfPortManagerAllocationStatusInvalidConfiguration
		} else {
		...
		if status != nnfv1alpha2.NnfPortManagerAllocationStatusInUse {
			unsatisfiedRequests++
		...
		if allocationStatus == nil {
			...
			if mgr.Status.Allocations == nil {
				mgr.Status.Allocations = make([]nnfv1alpha2.NnfPortManagerAllocationStatus, 0)
			}
...
func (r *NnfPortManagerReconciler) isAllocationNeeded(mgr *nnfv1alpha2.NnfPortManager, status *AllocationStatus) bool {
	if status.Status != nnfv1alpha2.NnfPortManagerAllocationStatusInUse && status.Status != nnfv1alpha2.NnfPortManagerAllocationStatusInsufficientResources {
		return false
	}
...
func (r *NnfPortManagerReconciler) cleanupUnusedAllocations(log logr.Logger, mgr *nnfv1alpha2.NnfPortManager, cooldown int) {
	...
	for idx := range mgr.Status.Allocations {
		...
		if !r.isAllocationNeeded(mgr, status) {
			...
			} else if status.Status == nnfv1alpha2.NnfPortManagerAllocationStatusCooldown {
...
func (r *NnfPortManagerReconciler) findAllocationStatus(mgr *nnfv1alpha2.NnfPortManager, spec AllocationSpec) *AllocationStatus {
	for idx := range mgr.Status.Allocations {
		status := &mgr.Status.Allocations[idx]
		if status.Requester == nil {
			continue
...
func (r *NnfPortManagerReconciler) isAllocated(mgr *nnfv1alpha2.NnfPortManager, spec AllocationSpec) bool {
	return r.findAllocationStatus(mgr, spec) != nil
}

// Find free ports to satisfy the provided specification.
func (r *NnfPortManagerReconciler) findFreePorts(log logr.Logger, mgr *nnfv1alpha2.NnfPortManager, config *dwsv1alpha2.SystemConfiguration, spec AllocationSpec) ([]uint16, nnfv1alpha2.NnfPortManagerAllocationStatusStatus) {

	portsInUse := make([]uint16, 0)
	for _, status := range mgr.Status.Allocations {
		if status.Status == nnfv1alpha2.NnfPortManagerAllocationStatusInUse ||
			status.Status == nnfv1alpha2.NnfPortManagerAllocationStatusCooldown {
	...
	if len(ports) >= count {
		...
		return ports[:count], nnfv1alpha2.NnfPortManagerAllocationStatusInUse
	}
	...
	claimPortsFromFreeAllocation := func() int {
		for idx := range mgr.Status.Allocations {
			...
			if status.Status == nnfv1alpha2.NnfPortManagerAllocationStatusFree {
	...
	for len(ports) < count {
		...
		case exhausted:
			return []uint16{}, nnfv1alpha2.NnfPortManagerAllocationStatusInsufficientResources
		case more:
...
func (r *NnfPortManagerReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&nnfv1alpha2.NnfPortManager{}).
		Complete(r)
}
