...
func (r *DWSServersReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	statusUpdater := updater.NewStatusUpdater[*dwsv1alpha7.ServersStatus](servers)
...
func (r *DWSServersReconciler) updateCapacityUsed(ctx context.Context, servers *dwsv1alpha7.Servers) (ctrl.Result, error) {
	...
	for storageIndex := range nnfStorage.Spec.AllocationSets {
		...
		if serversIndex == -1 {
			return ctrl.Result{}, dwsv1alpha7.NewResourceError("unable to find allocation label %s", label).WithFatal()
		}
		...
		for name, capacityAllocated := range capacityAllocatedMap {
			...
			servers.Status.AllocationSets[serversIndex].Storage[name] = dwsv1alpha7.ServersStatusStorage{AllocationSize: capacityAllocated, Ready: serverReady}
		}
...
func (r *DWSServersReconciler) clearAllocationStatus(servers *dwsv1alpha7.Servers) {
	servers.Status.AllocationSets = []dwsv1alpha7.ServersStatusAllocationSet{}
	for _, allocationSetSpec := range servers.Spec.AllocationSets {
		allocationSetStatus := dwsv1alpha7.ServersStatusAllocationSet{}
		allocationSetStatus.Label = allocationSetSpec.Label
		allocationSetStatus.Storage = make(map[string]dwsv1alpha7.ServersStatusStorage)
		for _, storage := range allocationSetSpec.Storage {
			allocationSetStatus.Storage[storage.Name] = dwsv1alpha7.ServersStatusStorage{AllocationSize: 0}
		}
...
func (r *DWSServersReconciler) statusSetEmpty(ctx context.Context, servers *dwsv1alpha7.Servers) (ctrl.Result, error) {
...
func (r *DWSServersReconciler) statusUpdate(ctx context.Context, servers *dwsv1alpha7.Servers, batch bool) (ctrl.Result, error) {
...
func (r *DWSServersReconciler) checkDeletedStorage(ctx context.Context, servers *dwsv1alpha7.Servers) (deletedStorage, error) {
	log := r.Log.WithValues("Servers", types.NamespacedName{Name: servers.Name, Namespace: servers.Namespace})
