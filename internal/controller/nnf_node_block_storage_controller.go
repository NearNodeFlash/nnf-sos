...
func (r *NnfNodeBlockStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	for i := range nodeBlockStorage.Spec.Allocations {
		...
		if err != nil {
			return ctrl.Result{}, dwsv1alpha6.NewResourceError("unable to allocate NVMe namespaces for allocation %v", i).WithError(err).WithMajor()
		}
		if result != nil {
			return *result, nil
		}
	...
	if _, found := os.LookupEnv("NNF_TEST_ENVIRONMENT"); !found {
		...
		if err := r.Get(ctx, client.ObjectKeyFromObject(pod), pod); err != nil {
			return ctrl.Result{}, dwsv1alpha6.NewResourceError("could not get pod: %v", client.ObjectKeyFromObject(pod)).WithError(err)
		}
		...
		for _, container := range pod.Status.ContainerStatuses {
			...
			if container.State.Running == nil {
				return ctrl.Result{}, dwsv1alpha6.NewResourceError("pod not in state running: %v", client.ObjectKeyFromObject(pod)).WithError(err).WithMajor()
			}
...
func (r *NnfNodeBlockStorageReconciler) allocateStorage(nodeBlockStorage *nnfv1alpha8.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	...
	if err != nil {
		return nil, dwsv1alpha6.NewResourceError("could not create storage pool").WithError(err).WithMajor()
	}
	...
	if len(allocationStatus.Devices) != len(vc.Members) {
		return nil, dwsv1alpha6.NewResourceError("unexpected number of namespaces").WithFatal()
	}
...
func (r *NnfNodeBlockStorageReconciler) createBlockDevice(ctx context.Context, nodeBlockStorage *nnfv1alpha8.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	...
	if err := ss.StorageServiceIdEndpointsGet(ss.Id(), serverEndpointCollection); err != nil {
		return nil, dwsv1alpha6.NewResourceError("could not get service endpoint").WithError(err).WithFatal()
	}
	...
	if err != nil {
		return nil, dwsv1alpha6.NewResourceError("could not read storage resource").WithError(err)
	}
		...
		if nodeName == "" {
			if _, err := r.getStorageGroup(ss, storageGroupId); err == nil {
				if err := r.deleteStorageGroup(ss, storageGroupId); err != nil {
					return nil, dwsv1alpha6.NewResourceError("could not delete storage group").WithError(err).WithMajor()
				}
			...
			if err != nil {
				return nil, dwsv1alpha6.NewResourceError("could not get endpoint").WithError(err).WithFatal()
			}
			...
			if err != nil {
				return nil, dwsv1alpha6.NewResourceError("could not create storage group").WithError(err).WithMajor()
			}
			...
			for i, allocatedDevice := range allocationStatus.Devices {
				...
				if path == "" {
					...
					if err != nil {
						return nil, dwsv1alpha6.NewResourceError("could not rescan devices after failing to find device path for %v", allocatedDevice).WithError(err).WithMajor()
					}

					return nil, dwsv1alpha6.NewResourceError("could not find device path for %v", allocatedDevice).WithMajor()
				}
...
func (r *NnfNodeBlockStorageReconciler) deleteStorage(nodeBlockStorage *nnfv1alpha8.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	...
	if err != nil {
		...
		if !ok || ecErr.StatusCode() != http.StatusNotFound {
			nodeBlockStorage.Status.Error = dwsv1alpha6.NewResourceError("could not delete storage pool").WithError(err).WithFatal()
...
func (r *NnfNodeBlockStorageReconciler) createStoragePool(ss nnf.StorageServiceApi, id string, capacity int64) (*sf.StoragePoolV150StoragePool, error) {
	...
	if err := ss.StorageServiceIdStoragePoolIdPut(ss.Id(), id, sp); err != nil {
		resourceErr := dwsv1alpha6.NewResourceError("could not allocate storage pool").WithError(err)
		ecErr, ok := err.(*ec.ControllerError)
		if ok {
			switch ecErr.Cause() {
