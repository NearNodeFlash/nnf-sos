...
func (r *NnfNodeBlockStorageReconciler) EventHandler(e nnfevent.Event) error {
	...
	r.Events <- event.GenericEvent{Object: &nnfv1alpha8.NnfNodeBlockStorage{
...
func (r *NnfNodeBlockStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	nodeBlockStorage := &nnfv1alpha8.NnfNodeBlockStorage{}
	...
	statusUpdater := updater.NewStatusUpdater[*nnfv1alpha8.NnfNodeBlockStorageStatus](nodeBlockStorage)
	...
	if len(nodeBlockStorage.Status.Allocations) == 0 {
		nodeBlockStorage.Status.Allocations = make([]nnfv1alpha8.NnfNodeBlockStorageAllocationStatus, len(nodeBlockStorage.Spec.Allocations))
		for i := range nodeBlockStorage.Status.Allocations {
			nodeBlockStorage.Status.Allocations[i].Accesses = make(map[string]nnfv1alpha8.NnfNodeBlockStorageAccessStatus)
		}
	...
	for i := range nodeBlockStorage.Spec.Allocations {
		...
		if err != nil {
			return ctrl.Result{}, dwsv1alpha5.NewResourceError("unable to allocate NVMe namespaces for allocation %v", i).WithError(err).WithMajor()
		}
		if result != nil {
			return *result, nil
		}
		...
		if err != nil {
			return ctrl.Result{}, dwsv1alpha5.NewResourceError("unable to attache NVMe namespace to node for allocation %v", i).WithError(err).WithMajor()
		}
		if result != nil {
			return *result, nil
		}
	...
	if _, found := os.LookupEnv("NNF_TEST_ENVIRONMENT"); !found {
		...
		if err := r.Get(ctx, client.ObjectKeyFromObject(pod), pod); err != nil {
			return ctrl.Result{}, dwsv1alpha5.NewResourceError("could not get pod: %v", client.ObjectKeyFromObject(pod)).WithError(err)
		}
		...
		for _, container := range pod.Status.ContainerStatuses {
			...
			if container.State.Running == nil {
				return ctrl.Result{}, dwsv1alpha5.NewResourceError("pod not in state running: %v", client.ObjectKeyFromObject(pod)).WithError(err).WithMajor()
			}
...
func (r *NnfNodeBlockStorageReconciler) allocateStorage(nodeBlockStorage *nnfv1alpha8.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	...
	if err != nil {
		return nil, dwsv1alpha5.NewResourceError("could not create storage pool").WithError(err).WithMajor()
	}
	...
	if len(allocationStatus.Devices) == 0 {
		allocationStatus.Devices = make([]nnfv1alpha8.NnfNodeBlockStorageDeviceStatus, len(vc.Members))
	}

	if len(allocationStatus.Devices) != len(vc.Members) {
		return nil, dwsv1alpha5.NewResourceError("unexpected number of namespaces").WithFatal()
	}
...
func (r *NnfNodeBlockStorageReconciler) createBlockDevice(ctx context.Context, nodeBlockStorage *nnfv1alpha8.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	log := r.Log.WithValues("NnfNodeBlockStorage", types.NamespacedName{Name: nodeBlockStorage.Name, Namespace: nodeBlockStorage.Namespace})
	ss := nnf.NewDefaultStorageService(r.Options.DeleteUnknownVolumes(), r.Options.ReplaceMissingVolumes())
	...
	if err := ss.StorageServiceIdEndpointsGet(ss.Id(), serverEndpointCollection); err != nil {
		return nil, dwsv1alpha5.NewResourceError("could not get service endpoint").WithError(err).WithFatal()
	}
	...
	if err != nil {
		return nil, dwsv1alpha5.NewResourceError("could not read storage resource").WithError(err)
	}
		...
		if nodeName == "" {
			if _, err := r.getStorageGroup(ss, storageGroupId); err == nil {
				if err := r.deleteStorageGroup(ss, storageGroupId); err != nil {
					return nil, dwsv1alpha5.NewResourceError("could not delete storage group").WithError(err).WithMajor()
				}
			...
			if os.Getenv("ENVIRONMENT") == "kind" && endpointID != os.Getenv("RABBIT_NODE") {
				allocationStatus.Accesses[nodeName] = nnfv1alpha8.NnfNodeBlockStorageAccessStatus{StorageGroupId: storageGroupId}
				continue
			}

			endPoint, err := r.getEndpoint(ss, endpointID)
			if err != nil {
				return nil, dwsv1alpha5.NewResourceError("could not get endpoint").WithError(err).WithFatal()
			}

			// Skip the endpoints that are not ready
			if nnfv1alpha8.StaticResourceStatus(endPoint.Status) != nnfv1alpha8.ResourceReady {
				continue
			}

			sg, err := r.createStorageGroup(ss, storageGroupId, allocationStatus.StoragePoolId, endpointID)
			if err != nil {
				return nil, dwsv1alpha5.NewResourceError("could not create storage group").WithError(err).WithMajor()
			}

			if allocationStatus.Accesses == nil {
				allocationStatus.Accesses = make(map[string]nnfv1alpha8.NnfNodeBlockStorageAccessStatus)
			}
			...
			if _, ok := allocationStatus.Accesses[nodeName]; !ok {
				...
				allocationStatus.Accesses[nodeName] = nnfv1alpha8.NnfNodeBlockStorageAccessStatus{StorageGroupId: sg.Id}
			}
			...
			for i, allocatedDevice := range allocationStatus.Devices {
				...
				if path == "" {
					...
					if err != nil {
						return nil, dwsv1alpha5.NewResourceError("could not rescan devices after failing to find device path for %v", allocatedDevice).WithError(err).WithMajor()
					}

					return nil, dwsv1alpha5.NewResourceError("could not find device path for %v", allocatedDevice).WithMajor()
				}
...
func (r *NnfNodeBlockStorageReconciler) deleteStorage(nodeBlockStorage *nnfv1alpha8.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	log := r.Log.WithValues("NnfNodeBlockStorage", types.NamespacedName{Name: nodeBlockStorage.Name, Namespace: nodeBlockStorage.Namespace})
...
func getStoragePoolID(nodeBlockStorage *nnfv1alpha8.NnfNodeBlockStorage, index int) string {
...
func (r *NnfNodeBlockStorageReconciler) createStoragePool(ss nnf.StorageServiceApi, id string, capacity int64) (*sf.StoragePoolV150StoragePool, error) {
	...
	if err := ss.StorageServiceIdStoragePoolIdPut(ss.Id(), id, sp); err != nil {
		resourceErr := dwsv1alpha5.NewResourceError("could not allocate storage pool").WithError(err)
		ecErr, ok := err.(*ec.ControllerError)
		if ok {
			switch ecErr.Cause() {
...
func (r *NnfNodeBlockStorageReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&nnfv1alpha8.NnfNodeBlockStorage{}).
