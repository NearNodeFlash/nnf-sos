...
func (r *NnfNodeBlockStorageReconciler) EventHandler(e nnfevent.Event) error {
	...
	r.Events <- event.GenericEvent{Object: &nnfv1alpha4.NnfNodeBlockStorage{
...
func (r *NnfNodeBlockStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	nodeBlockStorage := &nnfv1alpha4.NnfNodeBlockStorage{}
	...
	if len(nodeBlockStorage.Status.Allocations) == 0 {
		nodeBlockStorage.Status.Allocations = make([]nnfv1alpha4.NnfNodeBlockStorageAllocationStatus, len(nodeBlockStorage.Spec.Allocations))
		for i := range nodeBlockStorage.Status.Allocations {
			nodeBlockStorage.Status.Allocations[i].Accesses = make(map[string]nnfv1alpha4.NnfNodeBlockStorageAccessStatus)
		}
...
func (r *NnfNodeBlockStorageReconciler) allocateStorage(nodeBlockStorage *nnfv1alpha4.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	...
	if len(allocationStatus.Devices) == 0 {
		allocationStatus.Devices = make([]nnfv1alpha4.NnfNodeBlockStorageDeviceStatus, len(vc.Members))
	}
...
func (r *NnfNodeBlockStorageReconciler) createBlockDevice(ctx context.Context, nodeBlockStorage *nnfv1alpha4.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	log := r.Log.WithValues("NnfNodeBlockStorage", types.NamespacedName{Name: nodeBlockStorage.Name, Namespace: nodeBlockStorage.Namespace})
	ss := nnf.NewDefaultStorageService(r.Options.DeleteUnknownVolumes())
	...
	for clientIndex, nodeName := range accessList {
		...
		if nodeName == "" {
			...
			if os.Getenv("ENVIRONMENT") == "kind" && endpointID != os.Getenv("RABBIT_NODE") {
				allocationStatus.Accesses[nodeName] = nnfv1alpha4.NnfNodeBlockStorageAccessStatus{StorageGroupId: "fake-storage-group"}
				continue
			}
			...
			if nnfv1alpha4.StaticResourceStatus(endPoint.Status) != nnfv1alpha4.ResourceReady {
				continue
			}
			...
			if allocationStatus.Accesses == nil {
				allocationStatus.Accesses = make(map[string]nnfv1alpha4.NnfNodeBlockStorageAccessStatus)
			}
			...
			if _, ok := allocationStatus.Accesses[nodeName]; !ok {
				...
				allocationStatus.Accesses[nodeName] = nnfv1alpha4.NnfNodeBlockStorageAccessStatus{StorageGroupId: sg.Id}
			}
...
func (r *NnfNodeBlockStorageReconciler) deleteStorage(nodeBlockStorage *nnfv1alpha4.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	log := r.Log.WithValues("NnfNodeBlockStorage", types.NamespacedName{Name: nodeBlockStorage.Name, Namespace: nodeBlockStorage.Namespace})
...
func getStoragePoolID(nodeBlockStorage *nnfv1alpha4.NnfNodeBlockStorage, index int) string {
...
func (r *NnfNodeBlockStorageReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&nnfv1alpha4.NnfNodeBlockStorage{}).
