...
func (r *NnfNodeBlockStorageReconciler) EventHandler(e nnfevent.Event) error {
	...
	r.Events <- event.GenericEvent{Object: &nnfv1alpha11.NnfNodeBlockStorage{
...
func (r *NnfNodeBlockStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	nodeBlockStorage := &nnfv1alpha11.NnfNodeBlockStorage{}
	...
	statusUpdater := updater.NewStatusUpdater[*nnfv1alpha11.NnfNodeBlockStorageStatus](nodeBlockStorage)
	...
	if len(nodeBlockStorage.Status.Allocations) == 0 {
		nodeBlockStorage.Status.Allocations = make([]nnfv1alpha11.NnfNodeBlockStorageAllocationStatus, len(nodeBlockStorage.Spec.Allocations))
		for i := range nodeBlockStorage.Status.Allocations {
			nodeBlockStorage.Status.Allocations[i].Accesses = make(map[string]nnfv1alpha11.NnfNodeBlockStorageAccessStatus)
		}
...
func (r *NnfNodeBlockStorageReconciler) allocateStorage(nodeBlockStorage *nnfv1alpha11.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	...
	if len(allocationStatus.Devices) == 0 {
		allocationStatus.Devices = make([]nnfv1alpha11.NnfNodeBlockStorageDeviceStatus, len(vc.Members))
	}
...
func (r *NnfNodeBlockStorageReconciler) createBlockDevice(ctx context.Context, nodeBlockStorage *nnfv1alpha11.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	log := r.Log.WithValues("NnfNodeBlockStorage", types.NamespacedName{Name: nodeBlockStorage.Name, Namespace: nodeBlockStorage.Namespace})
	ss := nnf.NewDefaultStorageService(r.Options.DeleteUnknownVolumes(), r.Options.ReplaceMissingVolumes())
	...
	for clientIndex, nodeName := range accessList {
		...
		if nodeName == "" {
			...
			if os.Getenv("ENVIRONMENT") == "kind" && endpointID != os.Getenv("RABBIT_NODE") {
				allocationStatus.Accesses[nodeName] = nnfv1alpha11.NnfNodeBlockStorageAccessStatus{StorageGroupId: storageGroupId}
				continue
			}
			...
			if nnfv1alpha11.StaticResourceStatus(endPoint.Status) != nnfv1alpha11.ResourceReady {
				continue
			}
			...
			if allocationStatus.Accesses == nil {
				allocationStatus.Accesses = make(map[string]nnfv1alpha11.NnfNodeBlockStorageAccessStatus)
			}
			...
			if _, ok := allocationStatus.Accesses[nodeName]; !ok {
				...
				allocationStatus.Accesses[nodeName] = nnfv1alpha11.NnfNodeBlockStorageAccessStatus{StorageGroupId: sg.Id}
			}
...
func (r *NnfNodeBlockStorageReconciler) deleteStorage(nodeBlockStorage *nnfv1alpha11.NnfNodeBlockStorage, index int) (*ctrl.Result, error) {
	log := r.Log.WithValues("NnfNodeBlockStorage", types.NamespacedName{Name: nodeBlockStorage.Name, Namespace: nodeBlockStorage.Namespace})
...
func getStoragePoolID(nodeBlockStorage *nnfv1alpha11.NnfNodeBlockStorage, index int) string {
...
func (r *NnfNodeBlockStorageReconciler) getComputeNodesWithAccess(blockStorage *nnfv1alpha11.NnfNodeBlockStorage) []string {
...
func (r *NnfNodeBlockStorageReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	builder := ctrl.NewControllerManagedBy(mgr).
		...
		For(&nnfv1alpha11.NnfNodeBlockStorage{}).
