...
func (r *NnfStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	statusUpdater := updater.NewStatusUpdater[*nnfv1alpha11.NnfStorageStatus](storage)
	...
	if len(storage.Status.AllocationSets) != len(storage.Spec.AllocationSets) {
		storage.Status.AllocationSets = make([]nnfv1alpha11.NnfStorageAllocationSetStatus, len(storage.Spec.AllocationSets))
	...
	if storage.Spec.FileSystemType == "lustre" {
		...
		storage.Status.LustreComponents = nnfv1alpha11.NnfStorageLustreComponents{
			MDTs:     components["mdt"],
			MGTs:     components["mgt"],
	...
	overallHealth := nnfv1alpha11.NnfStorageHealthHealthy
	for i := range storage.Spec.AllocationSets {
		...
		if health != nnfv1alpha11.NnfStorageHealthHealthy {
...
func (r *NnfStorageReconciler) addPersistentStorageReference(ctx context.Context, nnfStorage *nnfv1alpha11.NnfStorage, persistentMgsReference corev1.ObjectReference) error {
...
func (r *NnfStorageReconciler) removePersistentStorageReference(ctx context.Context, nnfStorage *nnfv1alpha11.NnfStorage, persistentMgsReference corev1.ObjectReference) error {
	persistentStorage := &dwsv1alpha7.PersistentStorageInstance{
		ObjectMeta: metav1.ObjectMeta{
			Name:      persistentMgsReference.Name,
			Namespace: persistentMgsReference.Namespace,
...
func (r *NnfStorageReconciler) createNodeBlockStorage(ctx context.Context, nnfStorage *nnfv1alpha11.NnfStorage, allocationSetIndex int) (*ctrl.Result, error) {
	...
	for i, node := range allocationSet.Nodes {
		...
		result, err := ctrl.CreateOrUpdate(ctx, r.Client, nnfNodeBlockStorage,
			func() error {
				...
				labels[nnfv1alpha11.AllocationSetLabel] = allocationSet.Name
				...
				if len(nnfNodeBlockStorage.Spec.Allocations) == 0 {
					nnfNodeBlockStorage.Spec.Allocations = make([]nnfv1alpha11.NnfNodeBlockStorageAllocationSpec, expectedAllocations)
				}
...
// for the NnfStorage.
func (r *NnfStorageReconciler) aggregateNodeBlockStorageStatus(ctx context.Context, nnfStorage *nnfv1alpha11.NnfStorage, allocationSetIndex int) (*ctrl.Result, error) {
	...
	matchLabels[nnfv1alpha11.AllocationSetLabel] = nnfStorage.Spec.AllocationSets[allocationSetIndex].Name
...
func (r *NnfStorageReconciler) createNodeStorage(ctx context.Context, nnfStorage *nnfv1alpha11.NnfStorage, allocationSetIndex int) (*ctrl.Result, error) {
	log := r.Log.WithValues("NnfStorage", types.NamespacedName{Name: nnfStorage.Name, Namespace: nnfStorage.Namespace})
	...
	if nnfStorage.Spec.FileSystemType == "lustre" {
		...
		allocationSet := nnfStorage.Spec.AllocationSets[allocationSetIndex]
		if allocationSet.TargetType == "mgt" || allocationSet.TargetType == "mgtmdt" {
			nnfLustreMgt := &nnfv1alpha11.NnfLustreMGT{
				ObjectMeta: metav1.ObjectMeta{
					Name:      nnfStorage.GetName(),
					Namespace: mgsNode,
				},
				Spec: nnfv1alpha11.NnfLustreMGTSpec{
		...
		result, err := ctrl.CreateOrUpdate(ctx, r.Client, nnfNodeStorage,
			func() error {
				...
				if lustreOST && startIndex == 0 {
					labels[nnfv1alpha11.AllocationSetOST0Label] = "true"
				}
				...
				nnfNodeStorage.Spec.BlockReference = corev1.ObjectReference{
					Name:      nnfNodeStorageName(nnfStorage, allocationSetIndex, i),
					Namespace: node.Name,
					Kind:      reflect.TypeOf(nnfv1alpha11.NnfNodeBlockStorage{}).Name(),
				}
				...
				for _, commandVariable := range allocationSet.CommandVariables {
					newCommandVariable := nnfv1alpha11.CommandVariablesSpec{
...
func (r *NnfStorageReconciler) aggregateNodeStorageStatus(ctx context.Context, storage *nnfv1alpha11.NnfStorage, allocationSetIndex int, deleting, skipOST0 bool) (*ctrl.Result, error) {
	...
	matchLabels[nnfv1alpha11.AllocationSetLabel] = storage.Spec.AllocationSets[allocationSetIndex].Name
	...
	// prune out any entries that aren't in the NnfStorage. This can happen if the NnfStorage was modified
	// after it was created, as is the case with NnfStorages from an NnfSystemStorage
	nnfNodeStorages := []nnfv1alpha11.NnfNodeStorage{}
...
func (r *NnfStorageReconciler) aggregateNodeStorageHealth(ctx context.Context, storage *nnfv1alpha11.NnfStorage, allocationSetIndex int) (nnfv1alpha11.NnfStorageHealth, error) {
	nnfNodeStorageList := &nnfv1alpha11.NnfNodeStorageList{}
	matchLabels := dwsv1alpha7.MatchingOwner(storage)
	matchLabels[nnfv1alpha11.AllocationSetLabel] = storage.Spec.AllocationSets[allocationSetIndex].Name
	...
	if err := r.List(ctx, nnfNodeStorageList, listOptions...); err != nil {
		return nnfv1alpha11.NnfStorageHealthHealthy, dwsv1alpha7.NewResourceError("could not list NnfNodeStorages").WithError(err)
	}
	...
	storage.Status.AllocationSets[allocationSetIndex].Health = nnfv1alpha11.NnfStorageHealthHealthy
	for _, nnfNodeStorage := range nnfNodeStorages {
		if nnfNodeStorage.Status.Health != nnfv1alpha11.NnfStorageHealthHealthy {
	...
	return nnfv1alpha11.NnfStorageHealthHealthy, nil
}

func (r *NnfStorageReconciler) getLustreMgt(ctx context.Context, nnfStorage *nnfv1alpha11.NnfStorage) (*nnfv1alpha11.NnfLustreMGT, error) {
	...
	nnfLustreMgtList := &nnfv1alpha11.NnfLustreMGTList{}
	...
	var nnfLustreMgt *nnfv1alpha11.NnfLustreMGT = nil
...
func (r *NnfStorageReconciler) getFsName(ctx context.Context, nnfStorage *nnfv1alpha11.NnfStorage) (string, error) {
	...
	if nnfStorage.Status.LustreMgtReference == (corev1.ObjectReference{}) {
		nnfStorage.Status.LustreMgtReference = corev1.ObjectReference{
			...
			Kind:      reflect.TypeOf(nnfv1alpha11.NnfLustreMGT{}).Name(),
		}
...
func (r *NnfStorageReconciler) runSharedMGTCommands(ctx context.Context, nnfStorage *nnfv1alpha11.NnfStorage) (*ctrl.Result, error) {
	log := r.Log.WithValues("NnfStorage", client.ObjectKeyFromObject(nnfStorage))
	...
	mgtCommand := nnfv1alpha11.NnfLustreMGTSpecCommand{
		Reference: corev1.ObjectReference{
			Name:      nnfStorage.Name,
			Namespace: nnfStorage.Namespace,
			Kind:      reflect.TypeOf(nnfv1alpha11.NnfStorage{}).Name(),
...
func (r *NnfStorageReconciler) setLustreOwnerGroup(ctx context.Context, nnfStorage *nnfv1alpha11.NnfStorage) (*ctrl.Result, error) {
	log := r.Log.WithValues("NnfStorage", client.ObjectKeyFromObject(nnfStorage))
...
func getTempClientMountDir(nnfStorage *nnfv1alpha11.NnfStorage, index int) string {
	return fmt.Sprintf("/%s/%s", getTempMountDir(), nnfNodeStorageName(nnfStorage, index, 0))
}

// Get the status from all the child NnfNodeStorage resources and use them to build the status
// for the NnfStorage.
func (r *NnfStorageReconciler) aggregateClientMountStatus(ctx context.Context, storage *nnfv1alpha11.NnfStorage, deleting bool) error {
...
// to the NnfStorage.
func (r *NnfStorageReconciler) teardownStorage(ctx context.Context, storage *nnfv1alpha11.NnfStorage) (nodeStoragesState, error) {
	...
	childObjects := []dwsv1alpha7.ObjectList{
		&nnfv1alpha11.NnfNodeStorageList{},
		&nnfv1alpha11.NnfNodeBlockStorageList{},
	}

	if storage.Spec.FileSystemType == "lustre" {
		// Delete OST0 first so that PreUnmount commands can happen
		ost0DeleteStatus, err := dwsv1alpha7.DeleteChildrenWithLabelsParallel(ctx, r.Client, childObjects, storage, client.MatchingLabels{nnfv1alpha11.AllocationSetOST0Label: "true"})
		if err != nil {
			return nodeStoragesExist, err
		}
		...
		ostDeleteStatus, err := dwsv1alpha7.DeleteChildrenWithLabelsParallel(ctx, r.Client, childObjects, storage, client.MatchingLabels{nnfv1alpha11.AllocationSetLabel: "ost"})
		if err != nil {
			return nodeStoragesExist, err
		}
		...
		mdtDeleteStatus, err := dwsv1alpha7.DeleteChildrenWithLabelsParallel(ctx, r.Client, childObjects, storage, client.MatchingLabels{nnfv1alpha11.AllocationSetLabel: "mdt"})
		if err != nil {
			return nodeStoragesExist, err
		}
...
func (r *NnfStorageReconciler) releaseLustreMgt(ctx context.Context, storage *nnfv1alpha11.NnfStorage) (bool, error) {
	if storage.Spec.FileSystemType != "lustre" {
		return true, nil
	}
...
func nnfNodeStorageName(storage *nnfv1alpha11.NnfStorage, allocationSetIndex int, i int) string {
...
func (r *NnfStorageReconciler) getLustreOST0(ctx context.Context, storage *nnfv1alpha11.NnfStorage) (*nnfv1alpha11.NnfNodeStorage, error) {
	if storage.Spec.FileSystemType != "lustre" {
		return nil, nil
	}
	...
	nnfNodeStorageList := &nnfv1alpha11.NnfNodeStorageList{}
	matchLabels := dwsv1alpha7.MatchingOwner(storage)
	matchLabels[nnfv1alpha11.AllocationSetLabel] = "ost"
...
func getLustreMappingFromStorage(storage *nnfv1alpha11.NnfStorage) map[string][]string {
	nnfNodeKey := "nnfNode"
