...
var _ = Context("NNF Port Manager Controller Setup", Ordered, func() {
	...
	Describe("NNF Port Manager Controller Test", func() {
		...
		allocatePorts := func(mgr *nnfv1alpha1.NnfPortManager, name string, count int) []uint16 {
		...
		reservePortsAllowFail := func(mgr *nnfv1alpha1.NnfPortManager, name string, count int) []uint16 {
		...
		// of the test.
		kickPortManager := func(mgr *nnfv1alpha1.NnfPortManager) {
		...
		verifyNumAllocations := func(mgr *nnfv1alpha1.NnfPortManager, status nnfv1alpha1.NnfPortManagerAllocationStatusStatus, count int) {
		...
		verifyNumAllocationsInUse := func(mgr *nnfv1alpha1.NnfPortManager, count int) {
			verifyNumAllocations(mgr, nnfv1alpha1.NnfPortManagerAllocationStatusInUse, count)
		}

		verifyNumAllocationsCooldown := func(mgr *nnfv1alpha1.NnfPortManager, count int) {
			verifyNumAllocations(mgr, nnfv1alpha1.NnfPortManagerAllocationStatusCooldown, count)
		}

		verifyNumAllocationsInsuffientResources := func(mgr *nnfv1alpha1.NnfPortManager, count int) {
			verifyNumAllocations(mgr, nnfv1alpha1.NnfPortManagerAllocationStatusInsufficientResources, count)
		}
		...
		When("the system configuration is missing", func() {
			It("should have a status that indicates system configuration is not found", func() {
				...
				Eventually(func() nnfv1alpha1.NnfPortManagerStatusStatus {
					k8sClient.Get(ctx, client.ObjectKeyFromObject(mgr), mgr)
					return mgr.Status.Status
				}).Should(Equal(nnfv1alpha1.NnfPortManagerStatusSystemConfigurationNotFound))
			})
		})
		...
		When("reserving ports with portCooldown", func() {
			...
			When("all ports are already reserved and another allocation is requested", func() {
				It("should eventually free up the cooldown ports and successfully reserve", func() {
					...
					allocation := nnfv1alpha1.NnfPortManagerAllocationSpec{Requester: corev1.ObjectReference{Name: "waiting"}, Count: 1}
					status := r.findAllocationStatus(mgr, allocation)
