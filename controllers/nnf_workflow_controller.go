...
func (r *NnfWorkflowReconciler) startProposalState(ctx context.Context, workflow *dwsv1alpha2.Workflow, index int) (*result, error) {
	...
	if err := r.validateWorkflow(ctx, workflow); err != nil {
		return nil, dwsv1alpha2.NewResourceError("unable to validate DW directives").WithError(err).WithFatal()
	}
...
func (r *NnfWorkflowReconciler) finishSetupState(ctx context.Context, workflow *dwsv1alpha2.Workflow, index int) (*result, error) {
	...
	if err := r.Get(ctx, client.ObjectKeyFromObject(nnfStorage), nnfStorage); err != nil {
		return nil, dwsv1alpha2.NewResourceError("could not get NnfStorage: %v", client.ObjectKeyFromObject(nnfStorage)).WithError(err)
	}
	...
	if nnfStorage.Status.Error != nil {
		return nil, dwsv1alpha2.NewResourceError("storage resource error: %v", client.ObjectKeyFromObject(nnfStorage)).WithError(nnfStorage.Status.Error)
	}
...
func (r *NnfWorkflowReconciler) startDataInOutState(ctx context.Context, workflow *dwsv1alpha2.Workflow, index int) (*result, error) {
	...
	if err != nil {
		return nil, dwsv1alpha2.NewResourceError("Invalid DW directive: %v", workflow.Spec.DWDirectives[index]).WithFatal().WithUser()
	}
	...
	prepareStagingArgumentFn := func(param string) (*corev1.ObjectReference, *nnfv1alpha1.NnfAccess, *result, error) {
		...
		if strings.HasPrefix(param, "$DW_JOB_") || strings.HasPrefix(param, "$DW_PERSISTENT_") {
			...
			if parentDwIndex < 0 {
				return nil, nil, nil, dwsv1alpha2.NewResourceError("no directive matching '%v' found in workflow", name).WithFatal().WithUser()
			}
			...
			if err != nil {
				return storageReference, access, nil, dwsv1alpha2.NewResourceError("could not create data movement mount points").WithError(err)
			}
		...
		return nil, nil, nil, dwsv1alpha2.NewResourceError("Staging parameter '%s' is invalid", param).WithFatal().WithUser()
	}

	sourceStorage, sourceAccess, result, err := prepareStagingArgumentFn(dwArgs["source"])
	if err != nil {
		return nil, dwsv1alpha2.NewResourceError("could not prepare data movement resources").WithError(err)
	} else if result != nil {
		return result, nil
	}
			...
			for i := 0; i < node.Count; i++ {
				...
				if err := r.Create(ctx, dm); err != nil {
					if !errors.IsAlreadyExists(err) {
						return nil, dwsv1alpha2.NewResourceError("could not create DataMovement: %v", client.ObjectKeyFromObject(dm)).WithError(err)
					}
				}
			}
...
func (r *NnfWorkflowReconciler) finishDataInOutState(ctx context.Context, workflow *dwsv1alpha2.Workflow, index int) (*result, error) {
	...
	for _, dm := range dataMovementList.Items {
		if dm.Status.Status != nnfv1alpha1.DataMovementConditionReasonSuccess {
			return nil, dwsv1alpha2.NewResourceError("staging operation failed").WithFatal()
		}
	}
...
func (r *NnfWorkflowReconciler) startPreRunState(ctx context.Context, workflow *dwsv1alpha2.Workflow, index int) (*result, error) {
	...
	if err != nil {
		return nil, dwsv1alpha2.NewResourceError("Unable to determine directive file system type").WithError(err).WithFatal().WithUser()
	}
...
func (r *NnfWorkflowReconciler) finishPreRunState(ctx context.Context, workflow *dwsv1alpha2.Workflow, index int) (*result, error) {
	...
	default:
		return nil, dwsv1alpha2.NewResourceError("unexpected directive: %v", dwArgs["command"]).WithFatal()
	}
...
func (r *NnfWorkflowReconciler) finishTeardownState(ctx context.Context, workflow *dwsv1alpha2.Workflow, index int) (*result, error) {
	...
	case "create_persistent":
		...
		if err != nil {
			return nil, dwsv1alpha2.NewResourceError("could not find persistent storage %v", dwArgs["name"]).WithError(err)
		}
	...
	case "destroy_persistent":
		...
		if err != nil {
			if !apierrors.IsNotFound(err) {
				return nil, dwsv1alpha2.NewResourceError("could not find peristent storage %v", dwArgs["name"]).WithError(err).WithFatal().WithUser()
			}
	...
	case "persistentdw":
		...
		if err != nil {
			return nil, dwsv1alpha2.NewResourceError("Could not remove persistent storage reference").WithError(err)
		}
	default:
	}
