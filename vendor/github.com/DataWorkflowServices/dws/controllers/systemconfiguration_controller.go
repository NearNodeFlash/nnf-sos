...
func (r *SystemConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	if !systemConfiguration.GetDeletionTimestamp().IsZero() {
		...
		deleteStatus, err := dwsv1alpha6.DeleteChildren(ctx, r.Client, r.ChildObjects, systemConfiguration)
		if err != nil {
	...
	for _, storageNode := range systemConfiguration.Spec.StorageNodes {
		...
		result, err := ctrl.CreateOrUpdate(ctx, r.Client, storage,
			func() error {
				dwsv1alpha6.AddOwnerLabels(storage, systemConfiguration)
				labels := storage.GetLabels()
				labels[dwsv1alpha6.StorageTypeLabel] = storageNode.Type
		...
		if err != nil {
			return ctrl.Result{}, dwsv1alpha6.NewResourceError("CreateOrUpdate failed for storage: %v", client.ObjectKeyFromObject(storage)).WithError(err)
		}
...
func (r *SystemConfigurationReconciler) SetupWithManager(mgr ctrl.Manager) error {
	r.ChildObjects = []dwsv1alpha6.ObjectList{
		&dwsv1alpha6.StorageList{},
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dwsv1alpha6.SystemConfiguration{}).
		Owns(&dwsv1alpha6.Storage{}).
		Complete(r)
}
