...
# 2. Check if Node is Online (Unstandby)
echo "Checking node status..."
nodes_out=$($PCS_STATUS_CMD 2>&1)
echo "$nodes_out"

if echo "$nodes_out" | grep -qE "^[[:space:]]*Online:.*[[:space:]]$(hostname -s)([[:space:]]|$)"; then
    echo "Node is already Online."
    # Proceed to check locking services
else
    # Wait for node to be in Standby state before unstandbying
    # This handles the case where the node is "pending" or transitioning
    echo "Waiting for node to be in Standby state..."
    timeout=120
    elapsed=0
    while true; do
      nodes_out=$($PCS_STATUS_CMD 2>&1)
      ...
      if echo "$nodes_out" | grep -qE "^[[:space:]]*Standby:.*[[:space:]]$(hostname -s)([[:space:]]|$)"; then
          echo "Node is in Standby state."
          break
      fi

      # Check if it became Online while we were waiting
      if echo "$nodes_out" | grep -qE "^[[:space:]]*Online:.*[[:space:]]$(hostname -s)([[:space:]]|$)"; then
          echo "Node is already Online."
          break
      fi
      ...
      if [ $elapsed -ge $timeout ]; then
        echo "Timed out waiting for node to become Standby."
        # We don't exit here, we'll try to unstandby anyway, just in case
        break
      fi
      sleep 1
      elapsed=$((elapsed + 1))
    ...
    # Only unstandby if we are not already online
    if ! echo "$nodes_out" | grep -qE "^[[:space:]]*Online:.*[[:space:]]$(hostname -s)([[:space:]]|$)"; then
        echo "Unstandby node..."
        sudo pcs node unstandby

        # 3. Wait for Online state
...
# 4. Wait for Locking Services (DLM & lvmlockd)
